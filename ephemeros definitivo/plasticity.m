classdef plasticity < handle
    ...
    methods
        ...
        function init(obj)
            ...
            obj.video=VideoWriter("movie.mp3");
        ...
        function write(obj)
            ...
            % colorbar;
            % subplot(1,2,2);
            % obj.unk.z.plot(prc="norm",disp=obj.unk.u,scale=10,LineStyle="None");
            % colorbar;
            ...
            figure(1);
            clf;
            %obj.unk.z.plot(prc="norm",disp=obj.unk.u,scale=obj.setting.video.scale,LineStyle="None");
            obj.unk.p.plot(LineStyle="None");
            colorbar;
            %clim([0,0.035]);
        ...
        function [res,stiff]=assemble(obj)
            ...
            K=l^2*(Ad*dotu.val{2,3}+A*u.val{2,3});
            ...
            Tp=(Bd*dotz.val{2,1}+B*z.val{2,1})+reshape(Y,1,1,[]).*dG;
            Kp=l^2*(Bd*dotz.val{2,2}+B*z.val{2,2});
            ...
            int=int+l^2*(Ad+A*obj.dt)*squeeze(sum(reshape(sU{2,3},d,d,d,nU,1,[]).*reshape(sU{2,3},d,d,d,1,nU,[]),[1,2,3]));
            ...
            int=int+(Bd+B*obj.dt)*sum(reshape(sZ{2,1},d,d,nZ,1,[]).*reshape(sZ{2,1},d,d,1,nZ,[]),[1,2]);
            ...
            int=int+squeeze(l^2*(Bd+B*obj.dt)*sum(reshape(sZ{2,2},d,d,d,nZ,1,[]).*reshape(sZ{2,2},d,d,d,1,nZ,[]),[1,2,3]));
