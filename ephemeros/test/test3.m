...
Omega=geo.whole;
GammaD=Omega.bound.sub(@(x) (abs(x(2,:)+2)<geo.tol));
GammaDn=Omega.bound.sub(@(x) 0*(abs(x(2,:)-2)<geo.tol));
...
pl.dat.f=@(x,t) [-12*(1-cos(t/pi/2));-0.5*(1-cos(2*pi*t/2))]*ones(1,size(x,2));
pl.dat.g=@(x,t) 0*x;
pl.unit.strain=1e-3;
