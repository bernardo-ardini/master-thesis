...
Omega=geo.whole;
GammaD=Omega.bound.sub(@(x) false(1,size(x,2)));
GammaDn=Omega.bound.sub(@(x) (abs(x(1,:))<geo.tol)+(abs(x(2,:))<geo.tol));
...
pl.dat.f=@(x,t) 1*0.5*(1-cos(2*pi*t/2))*[0;-10]*(x(2,:)>0);
pl.dat.g=@(x,t) 0.5*(1-cos(2*pi*t/2))*[0;-1e-2]*(x(1,:)>L/2).*(abs(x(2,:)-1)<geo.tol);
pl.unit.strain=1e-2;
...
function [dG,d2G]=diss(dotz)
    ...
    r=reshape(r,1,1,size(dotz,3));
    dG=dotz./sqrt(r.^2+c^2);

    r=reshape(r,1,1,1,1,size(dotz,3));
    I=reshape(eye(d^2),[d,d,d,d]);
    I=tensorprod(I,ones(size(dotz,3),1));
    dotzdotz=reshape(dotz,1,1,d,d,size(dotz,3)).*reshape(dotz,d,d,1,1,size(dotz,3));
    d2G=I./sqrt(r.^2+c^2)-dotzdotz./sqrt(r.^2+c^2).^3;
end
pl.mat.diss=@diss;
