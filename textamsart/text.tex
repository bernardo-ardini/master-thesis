...
\]
Clearly $A$ is nonempty and convex. We show that $A$ and $\inter(\epi f)$ are disjoint. Indeed if by contradiction $(\alpha,v)\in A\cap\inter(\epi f)$ then
\[
...
\end{equation}
for all $(a,v)\in\inter(\epi f)$ and $(b,w)\in A$. If we fix $v=w\in\dom f\cap\dom g$, $b\in\mathbb{R}$ sufficiently small such that $(b,v)\in A$ and we let $a\to\infty$, we discover that $\gamma\le0$ necessarily. If $\gamma=0$ then inequality~\eqref{eqn:inequality-hahn-banach} and Hahn--Banach theorem imply that $\int(\dom f)$ and $\dom g$ are separated by an hyperplane. This in turn implies that $\dom f$ and $\dom g$ are disjoint. This is a contradiction so that $\beta<0$. In this case we set $\theta=|\beta|^{-1}f\zeta$ and from inequality~\eqref{eqn:inequality-hahn-banach} we get
\[
\begin{split}
f^*(\theta)&=\sup_{v\in V}\left(\dual{\theta,v}-f(v)\right)=\\
...
Let $f\colon\boldsymbol{V}\to(-\infty,\infty]$ be a proper, l.s.c., positively 1-homogeneous and convex function. Then
\begin{itemize}
...
\emph{Step 2.} We prove that for all $\xi\in\boldsymbol{V}^*$ it holds that $f^*(\xi)\in\left\{0,\infty\right\}$. Indeed let $\xi\in\boldsymbol{V}^*$ such that $f^*(\xi)<\infty$. Then since $f$ is positively 1-homogeneous, it holds $f^*(\xi)=2^{-1}f^*(\xi)$ and this implies~$f^*(\xi)=0$.

\emph{Step 3.} Let $v\in\dom f$ and $\xi\in\partial f(v)$. This is equivalent to the equality $f(v)+f^*(\xi)=\dual{\xi,v}$ which in turn implies that $f^*(\xi)<\infty$ and by previous step $f^*(\xi)=0$. Then $f(v)=\dual{\xi,v}$ and for all $w\in\boldsymbol{V}$
\[
...
\begin{equation}
\label{eqn:bv-space}
\text{$\boldsymbol{V}$ is a separable and reflexive Banach space}
\tag{S}
\end{equation}

\begin{equation}
\label{eqn:bv-dissipation-1}
\text{$\mathcal{R}$ and $\mathcal{V}$ from $\boldsymbol{V}$ into $[0,\infty)$ are continuous, convex and strictly positive in $\boldsymbol{V}\setminus\{0\}$}
\tag{D.1}
\end{equation}
...
and for $\eta>0$
\[
\Phi_\eta(v)=\eta^{-1}\Phi_1(\eta v)=\mathcal{R}(v)+\eta^{-1}\mathcal{V}(\eta v).
\]
It holds that $\Phi_\eta^*(\xi)=\eta^{-1}\Phi^*(\xi)$. Since $\Phi_1$ is convex and $\Phi_1(0)=0$ then for all $v\in\boldsymbol{V}$ the functions $\eta\mapsto\Phi_\eta(v)$ are increasing. Then it is well defined
\[
...
\text{$\mathcal{E}\colon D\times[0,T]\to\mathbb{R}$ with $D\subset\boldsymbol{V}$}
\tag{E.1}
\end{equation}
...
\parbox[c]{0.8\textwidth}{regularity property of $\mathcal{E}$, I will find a simplified version of the properties stated in the article}
...
\[
\mathfrak{p}(v,\xi)=\inf_{\epsilon>0}\left(\Phi_\epsilon(v)+\Phi_\epsilon^*(\xi)\right).
\]
