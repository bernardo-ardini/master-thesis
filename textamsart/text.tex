...
Then
\begin{equation*}
z(t)\le e^{\int_0^t\alpha(s)ds}\left(C+\int_0^t\beta(s)e^{-\int_0^s\alpha(r)dr}\right)\quad\text{for a.e. $t\in(0,T)$}.
\end{equation*}
\end{prop}
\begin{proof}
Let
...
\begin{equation*}
\dot Z(t)=\alpha(t)z(t)+\beta(t)\le\alpha Z(t)+\beta(t)\quad\text{for a.e. $t\in(0,T)$}.
\end{equation*}
...
\parbox[c]{0.8\textwidth}{$\mathcal{E}(\cdot,t)$ has compact sublevels for all $t\in[0,T]$}
\tag{$\mathcal{E}$.2}
\end{equation}
\begin{equation}
...
\begin{equation*}
\Phi_\eta(\dot u(t))+\Phi_\eta^*(-\partial_t\mathcal{E}(u_\eta(t),t))=-\dual{\partial_u\mathcal{E}(u_\eta(t),t),\dot u_\eta(t)}\quad\text{for a.e. $t\in(0,T)$}.
\end{equation*}
...
\end{equation*}
To write the time-rescaled energy identity we introduce the map $\mathfrak{P}_\eta\colon(0,\infty)\times V\times\mathbb{R}\times V^*\to[0,\infty]$ defined by
\begin{equation*}
\mathfrak{P}_\eta(a,v;\gamma,\xi)=a\Phi_\eta(a^{-1}v)+a\Phi_\eta^*(\xi)+a\gamma
\end{equation*}
and $\mathfrak{F}_\eta\colon[0,T]\times V\times[0,\infty]\times V\to[0,\infty]$ defined by
\[
...
\begin{equation*}
\mathcal{E}(\boldsymbol{u}_\eta(t),\boldsymbol{t}_\eta(t))+\int_s^t\mathfrak{F}_\eta(\boldsymbol{t}_\eta(r),\boldsymbol{u}_\eta(r);\dot{\boldsymbol{t}}_\eta(r),\dot{\boldsymbol{u}}_\eta(r))dr=\mathcal{E}(\boldsymbol{u}_\eta(s),\boldsymbol{t}_\eta(s))\quad\text{for all $0\le s<t\le S_\eta$}.
\end{equation*}
...
\end{equation}
Now let $\mathfrak{P}\colon[0,\infty)\times V\times\mathbb{R}\times V^*\to[0,\infty]$ defined by
\begin{equation*}
\mathfrak{P}(a,v;\gamma,\xi)=\begin{cases}
\mathcal{R}(v)+\iota_K(\xi)+a\gamma & \text{if $a>0$} \\
\mathfrak{p}(v,\xi) & \text{if $a=0$}.
\end{cases}
\end{equation*}
and $\mathfrak{F}\colon[0,T]\times V\times[0,\infty)\times V\to[0,\infty]$ defined by
\[
\mathfrak{F}(t,u;a,v)=\mathfrak{P}(a,v;-\partial_t\mathcal{E}(u,t),-\partial_u\mathcal{E}(u,t)).
\]
\begin{defi}
We say that a pair $(\boldsymbol{u},\boldsymbol{t})\colon[0,S]\to V\times[0,T]$ is an admissible parametrized curve if
\begin{itemize}
\item[(i)] $\boldsymbol{t}$ is non decreasing and absolutely continuous, $\boldsymbol{u}\in\text{AC}_\mathcal{R}([0,S],V)$ and $\sup_{t\in[0,S]}\boldsymbol{u}(s)<\infty$
...
\[
\mathfrak{F}[\boldsymbol{t},\boldsymbol{u};\dot{\boldsymbol{t}},\dot{\boldsymbol{u}}](s)=\begin{cases}
...
\begin{itemize}
\item[(i)] non degenerate if $\dot{\boldsymbol{t}}(s)+\mathcal{R}[\dot{\boldsymbol{u}}](s)$ for a.e.\ $s\in[0,S]$
...
\[
\mathcal{E}(\boldsymbol{u}(t),\boldsymbol{t}(t))+\int_s^t\mathfrak{F}[\boldsymbol{t},\boldsymbol{u};\dot{\boldsymbol{t}},\dot{\boldsymbol{u}}](r)dr=\mathcal{E}(\boldsymbol{u}(s),\boldsymbol{t}(s))\quad\text{for all $0\le s<t\le S$}.
\]
\end{defi}
\begin{lemma}
...
\]
where is implicit that if $s\notin G$ then $\mathfrak{e}(\boldsymbol{u}_n(s),\boldsymbol{t}_n(s))\norm{\dot{\boldsymbol{u}}_n(s)}=0$. Then up to a extracting a subsequence $(\boldsymbol{u}_n,\boldsymbol{u}_n)$ converges uniformly on $[0,S]$ to some $(\boldsymbol{u},\boldsymbol{t})\in\mathscr{A}([0,S],D_E\times[0,T])$. Moreover
\[
...
\text{QUI PROBABILMENTE VA AGGIUNTA ANCHE L'ULTIMA}
\end{gather*}
E POI BISOGNA COMPLETARE ANCHE IL RESTO DEL LEMMA
\end{lemma}

\begin{thm}[existence of parametrized solutions]
Let $u_\eta\in\text{AC}([0,T])$ be solutions to the doubly nonlinear equation~\eqref{eqn:doubly-nonlinear-rescaled} with fixed initial condition $u_0\in V$. Consider non decreasing and surjective absolutely continuous time rescalings $\boldsymbol{t}_\eta\colon[0,S]\to[0,T]$ and let $\boldsymbol{u}_\eta=u_\eta(\boldsymbol{t}_\eta)$. Suppose that
\begin{equation}
...
\end{equation}
for some $\boldsymbol{m}\in L^\infty((0,S),(0,\infty))$. Then there exists a sequence $\eta_n\to0$ such that $(\boldsymbol{u}_\eta,\boldsymbol{t}_\eta)$ converges uniformly on $[0,S]$ to a parametrized balanced viscosity solution $(\boldsymbol{u},\boldsymbol{t})\in\text{AC}([0,S],V\times[0,T])$.
\end{thm}
\begin{proof}
Arguing as in the proof of Lemma~\ref{lemma:dissipation-bound} one finds that there exists $E>0$ such that $\boldsymbol{u}_\eta(s)\in D_E$ for all $\eta>0$ and $s\in[0,S]$. Since also~\eqref{eqn:normalization-hp} then there exists $L>0$ such that $\norm{\dot{\boldsymbol{t}}_\eta+\mathfrak{f}(\boldsymbol{u}_\eta,\dot{\boldsymbol{u}}_\eta,\boldsymbol{t}_\eta)}_\infty\le L$. Then the hypotheses of Lemma~\ref{lemma:compactness-parametrized} are satisfied and we can find a sequence $\eta_n\to0$ such that $(\boldsymbol{u}_{\eta_n},\boldsymbol{t}_{\eta_n})$ converges uniformly on $[0,S]$ to some $(\boldsymbol{u},\boldsymbol{t})\in\mathscr{A}([0,S],V\times[0,T])$. Since $\mathcal{E}\in C^1(V\times[0,T])$ then
\begin{equation}
...
\end{equation}
Now the energy identity and the definition of $\mathfrak{f}$ yields
\[
\mathcal{E}(\boldsymbol{u}_{\eta_n}(t),\boldsymbol{t}_\eta(t))+\int_s^t\mathfrak{f}(\boldsymbol{u}_{\eta_n}(r),\dot{\boldsymbol{u}}_{\eta_n}(r),\boldsymbol{t}_{\eta_n}(r))dr\le\mathcal{E}(\boldsymbol{t}_{\eta_n}(s),\boldsymbol{t}_{\eta_n}(s))+\int_s^t\partial_t\mathcal{E}(\boldsymbol{u}_{\eta_n}(r),\boldsymbol{t}_{\eta_n}(r))dr
\]
for all $0\le s<t\le T$. Taking the inferior limit as $n\to\infty$ and taking into account~\eqref{eqn:energy-convergence},~\eqref{eqn:power-convergence} and the lower semicontinuity estimates of Lemma~\ref{lemma:compactness-parametrized}
\[
\mathcal{E}(\boldsymbol{u}(t),\boldsymbol{t}(t))+\int_s^t\mathfrak{f}(\boldsymbol{u}(r),\dot{\boldsymbol{u}}(r),\boldsymbol{t}(r))dr\le\mathcal{E}(\boldsymbol{u}(s),\boldsymbol{t}(s))+\int_s^t\partial_t\mathcal{E}(\boldsymbol{u}(r),\boldsymbol{t}(r))dr
\]
for all $0\le s<t\le T$.
\end{proof}
