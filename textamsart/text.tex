...
Let $K=\partial\mathcal{R}(0)$. By Proposition~\ref{prop:prop-pos-1-hom-convx} it follows that $\mathcal{R}=\sigma_K$ and $\mathcal{R}^*=\iota_K$. It simply to check that $\Phi_\eta^*(\xi)=\eta^{-1}\Phi^*(\xi)$. It is possible to obtain a more explicit representation formula for $\Phi^*$. By the inf-convolution duality formula
\begin{equation}
...
Let
\begin{equation*}
\xi_\eta(t)=-\partial_u\mathcal{E}(u_\eta(t),t)\quad\text{and}\quad \gamma_\eta(t)=-\partial_t\mathcal{E}(u_\eta(t),t).
\end{equation*}
The identity~\eqref{eqn:energy-identity} implies
\begin{equation}
\label{eqn:energy-inequality}
\mathcal{E}(u_\eta(t),t)+\int_s^t\mathfrak{p}(\dot u_\eta(t),\xi_\eta(t))dr\le\mathcal{E}(u_\eta(s),s)+\int_s^t\partial_t\mathcal{E}(u_\eta(t),t)dr
\end{equation}
...
\begin{equation*}
\int_0^T\mathfrak{p}(\dot u_\eta(t),\xi_\eta(t))dt\le C\quad\text{for all $\eta>0$}.
\end{equation*}
\end{lemma}
\begin{proof}
Since $\mathfrak{p}\ge0$ by~\eqref{eqn:power-control} then inequality~\eqref{eqn:energy-inequality} implies
\begin{equation*}
...
\begin{equation*}
\int_0^T\mathfrak{p}(\dot u_\eta(t),-\partial_u\mathcal{E}(u_\eta(t),t))dt\le1+\mathcal{E}(u_0,0)+CT(1+C_1).
\end{equation*}
This concludes the proof.
\end{proof}
Let $\boldsymbol{s}_\eta\colon[0,T]\to[0,\boldsymbol{S}_\eta]$ defined by
\begin{equation}
\label{eqn:energy-arc-lenght}
\boldsymbol{s}_\eta(t)=t+\int_0^t\mathfrak{p}(\dot u_\eta(s),-\partial_t\mathcal{E}(u_\eta(s),s))ds\quad\text{and}\quad\boldsymbol{S}_\eta=\boldsymbol{s}_\eta(T).
\end{equation}
Let also
\begin{equation*}
\boldsymbol{t}_\eta=\boldsymbol{s}_\eta^{-1}\colon[0,\boldsymbol{S}_\eta]\to[0,T],\quad\boldsymbol{u}_\eta=u_\eta(\boldsymbol{t}_\eta)\colon[0,\boldsymbol{S}_\eta]\to V
\end{equation*}
and
\begin{equation*}
\boldsymbol{\gamma}_\eta=\gamma_\eta(\boldsymbol{t}_\eta)\colon[0,\boldsymbol{S}_\eta]\to\mathbb{R},\quad\boldsymbol{\xi}_\eta=\xi_\eta(\boldsymbol{t}_\eta)\colon[0,\boldsymbol{S}_\eta]\to V^*.
\end{equation*}
To write the time-rescaled energy identity we introduce the map $\mathfrak{P}_\eta\colon(0,\infty)\times V\times\mathbb{R}\times V^*\to\mathbb{R}$ defined by
\begin{equation*}
\mathfrak{P}_\eta(a,v;\gamma,\xi)=a\Phi_\eta(a^{-1}v)+a\Phi_\eta^*(\xi)+a\gamma.
\end{equation*}
Then~\eqref{eqn:energy-identity} becomes
\begin{equation*}
\mathcal{E}(\boldsymbol{u}_\eta(t),\boldsymbol{t}_\eta(t))+\int_s^t\mathfrak{P}_\eta(\dot{\boldsymbol{t}}_\eta(r),\dot{\boldsymbol{u}}_\eta(r);\boldsymbol{\gamma}_\eta(r),\boldsymbol{\xi}_\eta(r))dr=\mathcal{E}(\boldsymbol{u}_\eta(s),\boldsymbol{t}_\eta(s))\quad\text{for all $0\le s<t\le\boldsymbol{S}_\eta$}.
\end{equation*}
Identity~\eqref{eqn:energy-arc-lenght} implies
\begin{equation*}
1=\dot{\boldsymbol{t}}_\eta+\mathfrak{p}(\dot{\boldsymbol{u}}_\eta,\boldsymbol{\xi}_\eta)\quad\text{a.e. in $(0,T)$}.
\end{equation*}
Now let $\mathfrak{P}_\eta\colon[0,\infty)\times V\times\mathbb{R}\times V^*\to\mathbb{R}$ defined by
\begin{equation*}
