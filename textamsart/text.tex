...
\begin{thm}
Let $V$ a Banach space and let $f\in C^1(V)$ and $K\subset V$ compact. Then $f$ is Lipchitz continuous on $K$.
\end{thm}
\begin{proof}
...
\end{prop}
\begin{proof}
\emph{Step 1.} Since $u$ is continuous then $u([0,T])$ is compact. Indeed let $u_n=u(t_n)$ be a sequence in $u([0,T])$. Then up to a subsequence $t_n\to t$ and by continuity $u_n=u(t_n)\to u(t)$.

\emph{Step 2.} Since $\mathcal{E}\in C^1(V\times[0,T])$ then by Weierstrass theorem $\mathcal{E}$ and $\partial_t\mathcal{E}$ is bounded 
\end{proof}
