...
\tag{$\Phi$.2}
\end{equation}
Notice that from Proposition~\ref{prop:prop-pos-1-hom-convx} it holds $\mathcal{R}(0)=0$.
\begin{equation}
...
\tag{$\Phi$.3}
\end{equation}
Notice that $\phi$ can be extended to an even $C^1(\mathbb{R})$ function.
...
\]
This implies that $\boldsymbol{u}_n$ are equicontinuous. Then up to extracting a subsequence
\[
