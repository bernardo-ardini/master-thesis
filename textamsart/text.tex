...
\begin{equation*}
\Phi_\eta(\dot u(t))+\Phi_\eta^*(-\partial_t\mathcal{E}(u(t),t))=-\dual{\partial_u\mathcal{E}(u(t),t),\dot u(t)}\quad\text{for a.e. $t\in(0,T)$}.
\end{equation*}
...
Let $E>0$ and $\boldsymbol{t}_n\in\text{AC}([0,S],[0,T])$ non decreasing and surjective. Let $u_n\in\text{AC}([0,S],D_E)$. Assume that there exists $L>0$ such that
\[
\dot{\boldsymbol{t}}_n(s)+\mathcal{R}[\dot u_n](s)+\mathfrak{e}(\boldsymbol{u}_n(s),\boldsymbol{t}_n(s))\norm{\dot{\boldsymbol{u}}_n(s)}\le L\quad\text{for a.e.\ $s\in(0,S)$}
\]
...
\[
\dot{\boldsymbol{t}}(s)+\mathcal{R}[\dot u](s)+\mathfrak{e}(\boldsymbol{u}(s),\boldsymbol{t}(s))\norm{\dot{\boldsymbol{u}}(s)}\le L\quad\text{for a.e.\ $s\in(0,S)$}
\]
and the following lower semicontinuity properties holds
\begin{gather*}
\liminf_{n\to\infty}\int_0^S\mathcal{R}[\dot{\boldsymbol{u}}_n](s)ds\ge\int_0^S\mathcal{R}[\dot{\boldsymbol{u}}](s)ds \\
\liminf_{n\to\infty}\int_0^S\mathfrak{e}(\boldsymbol{u}_n(s),\boldsymbol{t}_n(s))\norm{\dot{\boldsymbol{u}}_n(s)}ds\ge\int_0^S\mathfrak{e}(\boldsymbol{u}(s),\boldsymbol{t}(s))\norm{\dot{\boldsymbol{u}}(s)}ds \\
...
SCRIVERE LO STATEMENT
\end{thm}
\begin{proof}
Let $(\boldsymbol{u}_\eta,\boldsymbol{t}_\eta)\in\text{AC}([0,S],V)$ as defined previously. CAPIRE COSA METTERE PER $S$. Arguing as in the proof of Lemma~\ref{lemma:dissipation-bound} one finds that there exists $E>0$ such that $\boldsymbol{u}_\eta(s)\in D_E$ for all $\eta>0$ and $s\in[0,S]$. Since also~\eqref{eqn:normalization-rescaling} holds, the hypotheses of Lemma~\ref{lemma:compactness-parametrized} are satisfied. CONTINUARE QUESTA PROVA.
\end{proof}
