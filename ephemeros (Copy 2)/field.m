classdef field < handle
    properties
        ...
        dom % domain
        dof % dof(a) ath dof
        hdval % hdval{r}(I1,...,IN,i1,...,ir,e,g) derivative of the component (I1,...,IN) of the field along direction (i1,...,ir) computed at gauss point g of element e
    ...
    methods
        ...
        function plot(obj,r,prc,options)
            arguments
                obj 
                r 
                prc 
            ...
            coord=obj.geo.gauss{obj.space.m}.trace{obj.space.m}.coord;
            vals=obj.val{obj.space.m,r};
            vals=prc(vals);
            vals=vals(:);
            ...
            gcoord=obj.geo.coord;

            F=scatteredInterpolant(x,y,vals,'linear','nearest');
            vals=F(gcoord(:,1),gcoord(:,2));

            if ~isempty(options.displ)
                uvcoord=round(obj.geo.coord/obj.geo.tol)*obj.geo.tol;
                uncoord=round(options.displ.coord/obj.geo.tol)*obj.geo.tol;
                ...
                uva=uva(unind(uvind),:);
                uva=uva';
                uva=reshape(uva,[options.displ.dim,size(obj.geo.coord,1)]);
                uva=squeeze(uva);

                gcoord=gcoord+(options.scale*uva)';
            end

            trisurf(obj.geo.topol{end},gcoord(:,1),gcoord(:,2),vals);
            view(2);
        end

        function oldplot(obj,options)
            arguments
                obj
                options.r=1
                options.prc="val"
            ...
            if ~strcmp(options.displ,"none")
                uvcoord=round(obj.geo.coord/obj.geo.tol)*obj.geo.tol;
                uncoord=round(options.displ.space.coord/obj.geo.tol)*obj.geo.tol;
                
                [uvind,unind]=ismember(uvcoord,uncoord,'rows');

                uva=options.displ.dof;
                uva=reshape(uva,options.displ.space.num,[]);
                uva=uva(unind(uvind),:);
                uva=uva';
                uva=reshape(uva,[options.displ.space.basis.dim,size(obj.geo.coord,1)]);
                uva=squeeze(uva);

                coord=coord+(options.scale*uva)';
            end
