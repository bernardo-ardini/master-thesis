...
Let $Y=W^{1,q}((0,L),\mathbb{R}^2)$ with $q\in(??,\infty)$ and $Z=L^{q_H}((0,L))\cap W^{1,r}((0,L))$. Set $Q=Y\times Z$. PER ORA IL VINCOLO SUGLI ESPONENTI È CHE DEVONO ESSERE TUTTI MAGGIORI DI  E MINORI DI INFINITO E $\frac{1}{q}+\frac{1}{q_H}<1$.
...
We assume that
\[
\frac{1}{q}=\frac{1}{q_W}+\frac{1}{q_H}.
\]
...
\norm{y_1'}_q=\norm{y_1'P^{-1}P}_q\le\norm{y_1'P^{-1}}_{q_W}\norm{P}_{q_H}
\]
so that using Lemma~\ref{lemma:inverse-young} with $\rho=q_W/q$ one gets
\[
\norm{y_1'P^{-1}}_{q_W}^{q_W}\ge\frac{\norm{y_1'}_q^{q_W}}{\norm{P}_{q_H}^{q_W}}\ge\rho\delta^\frac{\rho-1}{\rho}\norm{y_1'}_q^q-(\rho-1)\delta\norm{P}_{q_H}^{q_H}
\]
...
\[
\begin{split}
\norm{y_2'}_q&\le\frac{1}{\gamma}\norm{GP}_q+\frac{1}{\gamma}\norm{g_2'(y_1,t)y_1'y_2}_q\le\frac{1}{\gamma}\norm{G}_{q_W}\norm{P}_{q_H}+\frac{\Gamma}{\gamma}\norm{y_1'y_2}_q\le\\
&\le\frac{1}{\gamma}\norm{G}_{q_W}\norm{P}_{q_H}+\frac{\Gamma}{\gamma}\norm{y_1'}_q\norm{y_2}_\infty.
\end{split}
\]
...
\]
On a sublevel of $\mathcal{E}$ the quantities $\norm{G}_{q_W}$, $\norm{P}_{q_H}$, $\norm{y_1'}_q$ and $\norm{y_2}_q$ are bounded, so that there exists $M>0$ such that
\[
\norm{y_2'}_q\le M(1+\epsilon\norm{y_2'}_q).
\]
Choosing $\epsilon$ small enough, it follows that $\norm{y_2'}_q$ is bounded on the sublevels of $\mathcal{E}$. This concludes the proof.
\end{proof}

\begin{prop}[convergence of elastic strain measures]
\label{prop:convergence-minors}
Let $q,s\in(1,\infty)$ such that $\frac{1}{q}+\frac{1}{s}\le1$. Let $y^k,y\in W^{1,q}((0,L),\mathbb{R}\times\mathbb{R})$ and $P_k,P\in L^s((0,L))$ such that
\begin{gather*}
y^k\rightharpoonup y\quad\text{in $W^{1,q}((0,L))$} \\
...
\]
Notice also that, up to extracting a subsequence, $y_2^k\to y_2$ in $C^0([0,L])$. Now the results follows from Lemma~\ref{lemma:lemmetto-cretino} below.
\end{proof}
\begin{lemma}
...
\end{multline*}
Since $f_k$ are bounded in $L^q((0,L))$, $h_k$ are bounded in $L^s((0,L))$ and $\frac{1}{q}+\frac{1}{s}=1$, then $f_kh_k\phi$ are bounded in $L^1((0,L))$ and $gf_k\phi$ are bounded in $L^{s^*}((0,L))$. Moreover $gh\phi\in L^{q^*}((0,L))$. Then the thesis follows.
\end{proof}
