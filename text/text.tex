...
Let $L>0$ be the length of the cylindrical bar. The macroscopic configuration of the bar is described by $u_i\colon(0,L)\to\mathbb{R}$ for $i\in\{1,2\}$. Here $u_1$ is the longitudinal displacement of the sections in the axis direction and $u_2$ is the quantity describing the transversal expansion of the sections. The configuration of the microscopic structure of the bar is described by $p\colon(0,L)\to\mathbb{R}$. We define $e_1=u_1'-p$ and $e_2=u_2+(d-1)^{-1}p$.
...
\]
Its maximum is reached in the middle section so we conclude that the Mandel stress reach yield strength here first. Then we claim that the region where there is a nonzero plastic distortion is an interval $(L/2-\epsilon,L/2+\epsilon)$ where $\epsilon\ge0$ is a function of time. Since the Mandel stress is positive we claim also that $p>0$ in that interval. We now focus on the half of the bar corresponding to interval $(0,L/2)$. On $(0,L/2-\epsilon)$ the solution is of the form
\[
\begin{cases}
...
\end{equation}
Substituting that identity into equation~\eqref{eqn:ode-u2} we have obtained a differential equation in $y$ of the form
\[
y''=ay+b
\]
where $a$ and $b$ are two constants. A solution which is symmetric with respect to $x=L/2$ is of the form
\[
...
\end{rmk}
We now show explicitly that there exists a solution $u_2\in C^2([0,L])$. To this aim we impose the continuity of $u_2$ and $u_2'$ at $x=L/2-\epsilon$ and solving a linear system we find the constants $C_1$ and $C_2$. We need only to find $\epsilon$. To determine it we consider the Mandel stress computed the elastic range at $x=L/2$. We call $F_Y$ the smallest value of $F$ for which that Mandel stress reach $Y$. For $F\le F_y$ we have $\epsilon=\eta(F)=0$. To determine $\epsilon$ when $F>F_Y$ we impose that $m(L/2-\epsilon)=Y$. Since the resultant equation cannot be solved analytically in $\epsilon$ we find $F$ as a smooth function $f=\colon[0,\epsilon_0)\to\mathbb{R}$ of $\epsilon$. Here
\[
\epsilon_0=\sup_{F\in[0,\infty)}\eta(F).
\]
We notice that by continuity $F_Y=f(0)$. We find
\[
...
\]
After a meticulous analysis of the analytical expression of $f$ we find that $\epsilon_0>L/2$. This means that when the load reach the value $F=f(L/2)$ to the plastic distortion satisfies $p>0$ in the entire interval $(0,L)$. An explicit expression of $\epsilon_0$ is not available. To get further insight on the solution we look at the limit $l\to0^+$.
\begin{prop}
In the limit $l\to0^+$ the solution $u_2$ that we have found converges at least pointwise on $(0,L)\times[0,T]$ to the homogeneous solution of Section~\ref{subsec:homogeneous-solution}. This convergence is also uniform on compact subsets of $(0,L)$. 
\end{prop}
\begin{proof}
It is a direct computation.
\end{proof}
Further information on the solution can be obtained looking at particular values of the parameters. We fix $d=3$, $L=1$, $l=0.1$, $\kappa=100$, $\mu=100$, $B=0.1$, $Y=1$.
\begin{figure}[p]
\includegraphics[width=0.48\textwidth]{figures/epsilon2F.png}
\hfill
\includegraphics[width=0.48\textwidth]{figures/epsilon2u2endmiddle.png}
\caption{The plot on the right show the function $f$. The plot on the right show the value of $u_2$ at $x=L/2$ (at the middle of the bar) and $x=L/100$ (near the end of the bar).}
\label{fig:epsilon2F}
\end{figure}
In Figure~\ref{fig:epsilon2F} on the left we reported the graph of $f$. We see that $\epsilon_0$, which correspond to the abscissa of the vertical asymptote, is slightly greater then $L/2$. On the right we report as a function of $\epsilon$ the value of $u_2$ computed at $x=L/2$ and $x=L/100$ that means at the middle and near the end of the bar respectively. This last plot suggest that the shrinkage of the cross section has no limit and happens everywhere along the bar. In particular we have evidences for the following claim.
\begin{clm}
The solution $u_2$ converges to $-\infty$ uniformly on compact sets contained in $(0,L)$.
\end{clm}
\begin{rmk}
From the information obtained so far, we conclude that the small deformation model is not able to predict necking. Indeed the shrinkage of the cross section seems to be not localized on a small region around the middle of the bar but to be distributed along its entire length.
\end{rmk}
For completeness in Figure~\ref{fig:plotu2p} we report the plot of $u_2$ and $p$ when $\epsilon=0.3$. In Figure~\ref{fig:bar} we report a picture of the bar in the same situation. On the top there is the result obtained with a purely elastic bar while on the top the result predicted including the plastic behavior. The displacements of cross sections is magnified with a factor 7. The color represents the variable $p$.
\begin{figure}[p]
\centering
...
\[
\tilde{\mathcal{P}}=\int_\Omega f\cdot\tilde vdx+\int_{\Gamma_N}h\cdot\tilde vda.
\]
For simplicity we assume that no external torque is applied to the body and that there is no external action on the microscopic configuration. (Maybe these external actions could be added to describe better the boundary conditions imposed on the body.) 
...
\begin{rmk*}
I reported the local form of the equilibrium in the case $K=0$ only. I will report the general case following~\cite{gurtin-fried}.
\end{rmk*}
\begin{lemma}
The local form of the equilibrium condition is
\begin{align*}
&\diver(T+T_e)+f=0\quad\text{in $\Omega$} \\
&(T+T_e)n=h\quad\text{in $\Gamma_N$} \\
&-\dev T_e+T_p-\diver K_p=0\quad\text{in $\Omega$} \\
&p=-\frac{1}{d}\tr T_e\quad\text{in $\Omega$} \\
&K_p n=0\quad\text{in $\partial\Omega$}.
\end{align*}
\end{lemma}
\begin{proof}
We can write the virtual power as
\begin{multline*}
\tilde{\mathcal W}=-\int_\Omega\diver(T+T_e):\tilde vdx+\int_{\Gamma_N}(T+T_e)n\cdot\tilde vdx+\int_\Omega(T_p-\dev T_e-\diver K_p):\tilde V_pdx+\\
+\int_{\partial\Omega}K_pn:\tilde V_pdx.
\end{multline*}
Then the results follows from the fundamental theorem of the calculus of variations.
\end{proof}
...
\]
Let now $\mathcal{W}$ the internal power absorbed by the portion $\Lambda$. We define the dissipation rate in $\Lambda$ as
\[
...
\]
with $\mathfrak{C}$, $\mathfrak{A}$ and $\mathfrak{B}$ a symmetric and positive definite linear operators on $\text{Sym}$, $\text{Lin}\otimes\mathbb{R}$ and $\text{DevPla}$ respectively. The macroscopic response is described by
\[
...
\[
\mathbb{T}_p=\mathfrak{B}\mathbb{U}_p+\mathfrak{F}\dot{\mathbb{U}}_p+Y(p)\partial_{\dot U_p}G(\dot U_p)
\]
...
\item[(i)] a subdifferentiable dissipation potential $\Phi\colon\boldsymbol{Z}\to[0,\infty]$ satisfying $\Phi(0)=0$,
\end{itemize}
and we define
\[
\mathscr{W}(z,\dot z)=\partial\Psi(z)+\partial\Phi(\dot z).
\]
\begin{prop}
Let $z\colon[0,T]\to\boldsymbol{Z}$ be a process satisfying the equilibrium condition
\[
\partial\Psi(z(t))+\partial\Phi(\dot z(t))\ni\mathcal{P}(t)
\]
...
\end{prop}
\begin{proof}
Let $\xi(t)\in\partial\Phi(\dot z(t))$ such that $\partial\Psi(t)+\xi(t)=\mathcal{P}(t)$ for all $t\in[0,T]$. Then
\[
\mathcal{D}=\dual{\partial\Psi(z),\dot z}+\dual{\xi,\dot z}-\frac{d}{dt}\Psi(z)=\dual{\xi,\dot z}.
\]
But by definition of subdifferential we have $\Phi(0)\ge\Phi(\dot z)-\dual{\xi,\dot z}$ so that $\dual{\xi,\dot z}\ge\Phi(\dot z)\ge0$. This completes the thesis.
\end{proof}
\begin{rmk}
The inequality $\mathscr{D}\ge0$ is known as the dissipation inequality and has the following meaning: the power $\dual{\mathscr{W},\dot z}$ absorbed by the system is stored reversibly as free energy up to a positive irreversible dissipation.
\end{rmk}
...
\paragraph{Kinematics of constrain} Let $\boldsymbol{\Gamma}$ be a vector space. We consider the constrain $\mathcal{C}(z)=0$ where $\mathcal{C}\colon\boldsymbol{Z}\to\boldsymbol{\Gamma}^*$ is a subdifferentiable function. We notice that the constrain can be written as
\[
\dual{\mathcal{C}(z),\tilde\delta}=0\quad\text{for all $\tilde\delta\in\boldsymbol{\Gamma}$}.
\]

\paragraph{Constrain reaction} In addiction to the internal and external actions, when we treat a constrain with the method of Lagrange multipliers we must add also the constrain reaction $\mathcal{R}\in\boldsymbol{Z}^*$. We make the assumption that the power expanded by the reaction on velocities compatible with the constrain.
