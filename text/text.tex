...
The introduction of the non-associate flow law involves additional mathematical complications because it breaks the standard variational structure of classical plasticity theory; recently~\cite{morafrancfort,StefanelliVikelis2024} it has been demonstrated that non-associated plasticity actually admits a variational formulation and existence results have been obtained.

%Fino agli anni cinquanta l'applicazione della teoria della plasticità è rimasta confinata alla descrizione del comportamento meccanico dei metalli. Nel 1952, Drucker e Prager~\cite{DruckerPrager1952} estesero la teoria per descrivere il comportamento meccanico dei suoli, interprentando il classico criterio sulla resistenza limite delle terre di Mohr--Coulomb~\cite{Coulomb1776,Mohr1900} come un criterio di snervamento plastico. Inoltre proposero un nuovo criterio di snernervamento, oggi appunto noto come criterio di Drucker--Prager.
%
%La caratteristica del criterio di Drucker--Prager, assente nella plasticità classica dei metalli, è che la resistenza allo snervamento cresce con l'aumentare della pressione. Questo comportamento è opportuno per descrivere il comportamento plastico di materiali come i suoli per via della loro natura granulare. In effetti in questi casi il comportamento plastico è dovuto allo scorrimento dei grani di cui il materiale è composto e tale scorrimento viene ostacolato dall'attrito interno tra i grani quanto più essi sono premuti gli uni contro gli altri. Come abbiamo visto nella Sezione~\ref{sec:plasticity-review} del Capitolo~\ref{chap:modeling}, nella teoria della plasticità classica, fissato l'insieme di ammissibilità per lo stress, la legge di flusso dello strain plastico viene automaticamente determinata dalla condizione di normalità del legge del flusso plastico (si veda la Nota~\ref{rmk:flow-rule} e la discussione che segue): si dice che la legge di flusso è \emph{associata}. Nel caso cui il criterio di ammissibilità tiene conto anche della pressione, la condizione di normalità impone che lo stain plastico non possa rimanere deviatorico, per cui l'ipotesi di incomprimibilità plastica va abbondonato. Questo corrisponde in realtà ad un fenomeno meccanico reale chiamato \emph{dilatanza} nel contesto della geotecnica e ha una precisa interpretazione micromeccanica: quando avviene il flusso plastico i grani scorrono gli uni sugli altri e nel farlo passano da una configurazione di impacchettamento più ottimale ad una meno ottimale provocando l'aumento del volume. Tuttavia la legge di flusso associata sovrastima la dilatanza; per questo ovviare a questo è nono state sviluppate delle leggi di flusso non associate, in cui cioè la legge di flusso plastico è scelta indipendentamente dall'insieme di ammissibilità dello stess.
%
%L'introduzione della legge di flusso non associata comporta delle complicazioni matematiche aggiuntive perché rompe la struttura variazionale standard della teoria della plasticità classica; recentemente~\cite{morafrancfort,StefanelliVikelis2024} è stato dimostrato che la plasticità non associata ammette in realtà una formulazione variazionale e sono stati ottenuti dei risultati di esistenza.

Noi riteniamo che la legge di flusso non associata 

Questa parte della tesi è dedicata allo studio di questi modelli di plasticità in cui la resistenza allo snervamento è influenzata dalla pressione. In questo capitolo introduttivo descriveremo più in dettaglio il modello di Drucker--Prager e i medelli di flusso non associati

Let $\Omega\subset\mathbb{R}^d$, open, bounded and Lipschitz, be the reference configuration of the body. Assume that the configuration of the body is described by the couples $z=(u,E_p)$ with $u\colon\Omega\to\mathbb{R}^d$ the displacement field and $E_p\colon\Omega\to\text{Sym}$ the plastic strain. 

\chapter{Small deformation model}

\section{Formulation}

\textbf{Scrivere una introduzione}
