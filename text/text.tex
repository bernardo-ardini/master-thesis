...
This results suggest to define energetic solutions directly as evolutions $q\colon[0,T]\to Q$ satisfying~\eqref{eqn:energetic:motivation-definition} also when less regularity is assumed. This is discussed below.

\subsection{Introduction}

Let $Q$ be a separable and reflexive Banach space and $\mathcal{E}\colon Q\times[0,T]\to[0,\infty]$. Rather than using a dissipation potential, we describe the system's dissipation in terms of a dissipation distance. This is defined as a map $D\colon Q\times Q\to[0,\infty]$, where $D(q_0,q_1)$ denotes the minimum energy dissipated by the system when it jumps from $q_0$ to $q_1$. The dissipation along an evolution $q\colon[0,T]\to Q$ in the time interval $[s,t]$ is defined as
\[
...
The evolution problem associated with the model described in this chapter is of the form of an evolutionary variational inequality that has been studied in Section~\ref{sec:evi}. According to~\eqref{eqn:necking-small:free-energy-density}, the free energy $\Psi\colon Z\to\mathbb{R}$ is defined by
\[
\Psi(z)=
\begin{multlined}[t]
...
\begin{thm}
Let $\Phi\colon X\to\mathbb{R}\cup\mathbb{\infty}$ be convex, proper and lower semicontinuous. Assume that for all $\epsilon>0$ there exist $\Phi_\epsilon\colon X\to\mathbb{R}$ convex and G\^ateaux differentiable with $\Phi_\epsilon'\colon X\to X^*$ continuous and bounded, such that the following properties holds:
\begin{itemize}
...
\]
\end{itemize}
Let also $A\colon X\to X^*$ continuous, bounded and pseudomonotone. Moreover assume that the following coercivity property holds: there exists $y\in\dom\Phi$ such that for all $z\in X$ the inequality
\begin{equation}
