...
\begin{prop}
In the limit $l\to0^+$ the solution $u_2$ that we have found converges at least pointwise on $(0,L)\times[0,T]$ to the homogeneous solution of Section~\ref{subsec:homogeneous-solution}.
\end{prop}
\begin{proof}
...
\label{fig:epsilon2F}
\end{figure}
In Figure~\ref{fig:epsilon2F} on the left we reported the graph of $f$. We see that $\epsilon_0$, which correspond to the abscissa of the vertical asymptote, is slightly greater then $L/2$. On the right we report as a function of $\epsilon$ the value of $u_2$ computed at $x=L/2$ and $x=L/100$ that means at the middle and at the end of the bar respectively. This last plot suggest that the shrinkage of the cross section has no limit and happens everywhere along the bar.
\begin{rmk}
