...
\begin{rmk}
Let the dissipation distance $D\colon\mathbb{R}\to[0,\infty]$ be defined by
\[
...
Let $Y=W^{1,q_Y}((0,L),\mathbb{R}^2)$ with $q_Y\in(1,\infty)$ the space of $y$ and $Z=L^{q_H}((0,L))\cap W^{1,r_H}((0,L))$ with $q_H,r_H\in(1,\infty)$ the space of $P$. Set $Q=Y\times Z$.
...
The elastic free energy density is supposed to satisfy
\begin{itemize}[itemsep=1em]
...
Assume that $g_2'=0$. Then there exists $\mathscr{D}\subset\mathscr{Q}$ such that $\dom\mathcal{E}(\cdot,t)=\mathscr{D}$ for all $t\in[0,T]$. Moreover
\begin{itemize}
...
QUI O NELL'INTRODUZIONE, SCRIVERE QUALCOSA PER MOTIVARE LA NOZIONE DI ENERGETIC SOLUTIONS
