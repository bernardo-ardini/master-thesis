...
\]
where $W_e$ and $W_p$ form $\text{Sym}$ to $\mathbb{R}$ are the elastic and plastic free energy which are supposed to be differentiable. This gives the constitutive relation for the elastic stress $T_e=W_e'(E_e)$. If we split the plastic force $T_p$ as the sum of a conservative back-stress $B_p$ (for example $B_p=\mathfrak{B}E_p$, with $\mathfrak{B}>0$ and a dissipative plastic force $A_p$, then $B_p=W_p'(E_p)$. Notice that, having written $T_p=A_p+B_p$, the equilibrium of microforces can be written as $A_p=T_e-B_p$ in $\Omega$. To characterize the plastic response
\begin{itemize}
\item[(i)] we require the admissibility condition $A_p\in C$ where $C=\{f\le0\}\subset\text{Sym}$, with $f\colon\text{Sym}\to\mathbb{R}$ a convex \emph{yield function}, and
\item[(ii)] we impose the flow rule $\dot E_p=0$ if $A_p\in\inter C$ and $\dot E_p$ belongs to the normal outer cone to $\{A\in\text{Sym}\;|\;g(A)\le g(A_p)\}$ at $A_p$ if $A_p\in\partial C$, where $g\colon\text{Sym}\to\mathbb{R}$ is a convex \emph{plastic potential}.
\end{itemize}
...
\end{lemma}
\begin{proof}
It is trivial to see that $A_p\in\hat{C}(A_p)$ if and only if $f(A_p)\le 0$, or equivalently $A_p\in C$. Moreover $A_p\in\inter\hat{C}(A_p)$ if and only if $A_p\in\inter C$ and $A_p\in\partial\hat{C}(A_p)$ if and only if $A_p\in\partial C$. This implies also that if $A_p\in\inter C$, then $N_{\hat{C}(A_p)}(A_p)=\{0\}$, and if $A_p\in\partial\hat{C}(A_p)$ then 
\end{proof}
