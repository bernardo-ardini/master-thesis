...
Let $Y=W^{1,q}((0,L),\mathbb{R}^2)$ with $q\in(??,\infty)$ and $Z=L^{q_H}((0,L))\cap W^{1,r}((0,L))$. Set $Q=Y\times Z$.

\begin{equation}
\tag{G}
\parbox{0.8\textwidth}{$g_1\in C^1(\mathbb{R})$, $g_2\in C^1(\mathbb{R},(0,\infty))$ and there exists $0<\gamma<\Gamma<\infty$ for $i\in\{1,2\}$ such that $\gamma<g_1'<\Gamma$, $\gamma<g_2<\Gamma$ and $|g_2'|\le\Gamma$.}
\end{equation}
...
\tag{H0}
\\
\parbox{0.8\textwidth}{there exist $C_H>0$ and $q_H,r\in(1,\infty)$ such that $H(P,P')\ge C_H(-1+|P|^{q_H}+|P|^{-q_H}+|P'|^r)$ for all $P,P'\in\mathbb{R}$}
\tag{H1}
\\
\parbox{0.8\textwidth}{$H(P,P')=\infty$ if $P\le0$}
\tag{H2}
\end{gather}
...
\]
where now $\rho=1+q_W/q_H$. Then
\begin{equation}
\label{eqn:bound-y2}
\norm{g_2(y_1,t)y_2P}_{q_W}^{q_W}\ge\gamma^{q_W}\rho\delta^\rho\norm{y_2}_q^q-\gamma^{q_W}\frac{\rho}{\rho^*}\delta^{\rho+\rho^*}\norm{P^{-1}}_{q_H}^{q_H}.
\end{equation}
