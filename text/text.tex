...
Consider $N$ material points in $\mathbb{R}^d$. Their configuration can be described by matrices $x\in\mathbb{R}^{d\times N}$, where $x_{i\alpha}$ is the $i$th component of the position vector of the $\alpha$th material point.
\end{ex}
\begin{ex}
...
A generalized force can be seen as an element $F\in(\mathbb{R}^{d\times N})^*=\mathbb{R}^{d\times N}$. The virtual power is
\[
\dual{F,\tilde v}=\sum_{\alpha=1}^N\sum_{i=1}^dF_{i\alpha}\tilde v_{i\alpha}.
\]
In particular $F_{i\alpha}$ is the $i$th component of the force acting on the $\alpha$th material point. Since the following consideration is useful for understanding more complex models, let $N=2$, set $\tilde v_i\in\mathbb{R}^d$ with $(\tilde v_\alpha)_i=\tilde v_{\alpha i}$, and consider
\[
\dual{F,\tilde{v}}=f\cdot\left(\tilde v_1-\tilde v_2\right)
\]
for some $f\in\mathbb{R}^d$. This generalized force $F$ can represents the situation in which the second point exert a force $f$ on the first, and the first point exert a reaction force $-f$ on the second. 
\end{ex}
...
\end{ex}
Consider now a time interval $[0,T]$ and an actual evolution of the system $u\colon[0,T]\to V$. For simplicity, in this discussion we assume that $u\in\text{AC}([0,T],V)$ even thought less regular evolutions, for example exhibiting jumps, can be considered as we will see subsequently in this thesis. Along its evolution, the mechanical system is subject to various generalized forces and we call $\mathcal{F}\colon[0,T]\to V^*$ their sum. We require that equilibrium holds, that is
\[
...
\[
\Phi(\chi)=\int_\Omega W(X,\Grad\chi(X))dX,
\]
...
\begin{multline*}
\tilde{\mathcal{I}}=\int_0^L\left[P^{-1}\sigma_1\tilde v_1'+P^{\frac{1}{d-1}}\sigma_2\tilde v_2+P^{-1}\zeta\tilde v_2'\right]dX+\\
...
\[
\mathcal{E}(\overline{y}(t),\overline{P}(t),t)\le\mathcal{E}(\hat y,\hat P,t)+\mathcal{D}(P,\hat P)\quad\text{for all $(\hat y,\hat P)\in\mathscr{Q}$}.
\]
We actually start with the treatment of stability when only the elastic response is considered.
\begin{rmk}
...
We now turn to the study of the stability of the homogeneous solution $(\overline{\chi})$. To this aim we should check whether the second variation of the energy becomes indefinite for $\eta$ large enough. This can be done as follows: we insert a perturbation of the homogeneous solution $\chi_1=\overline\chi_1+v_1$ and $\chi_2=\overline\chi_2+v_2$ into problem~\eqref{eqn:definite-equations-necking-large-elastic}, then we linearize the resulting problem around the homogeneous solution and we look for nontrivial solutions. The linearized problem reads as follows
\begin{subequations}
...
\]
Taking into account the boundary condition $v_2'(X)=0$  for $X\in\{0,L\}$, we find that a nontrivial solution exists if and only if $\lambda=k\pi$ for $k\in\{1,2,\dots\}$ and is of the form
\[
...
\[
v_1(X)=\left(-\frac{\overline W_{12}}{\overline W_{11}}\frac{\overline W_{12}K_1}{\frac{\overline{W}_{12}^2}{\overline{W}_{11}}-\overline W_{22}}+1\right)K_1X+\frac{K_2}{\sqrt{\lambda}}\sin\left(\sqrt{\lambda}X\right)+K_3
\]
with $K_3\in\mathbb{R}$. Boundary condition imposes that $K_1=0$ and $K_3=0$.

This requires that $\lambda\ge0$. If $\lambda<0$ the only solution is $v_2=0$.

To study if these equations together with boundary conditions~\eqref{eqn:perturbed-bound} admits a nontrivial solution we first need the following general discussion.
...
\begin{subequations}
\begin{gather}
\overline W_{33}u_2'''+\left(\frac{\overline W_{12}^2}{\overline W_{11}}-\overline W_{22}\right)u_2'=0 \quad\text{in $(0,L)$}
\end{gather}
\end{subequations}
with boundary conditions
\begin{gather}
u_1(X)=0\quad\text{for $X\in\{0,L\}$}
u_2'(X)=0\quad\text{for $X\in\{0,L\}$}
\end{gather}
Let $\xi=u_2'$ and
\[
\lambda=\frac{\frac{(\overline W_{12})^2}{\overline W_{11}}-\overline W_{22}}{\overline W_{33}}.
...
\begin{rmk}
Let $F^*=W_1^*$ be the axial force obtained in the homogeneous solution. Then
\[
...
Now we show that there exists a particular constitutive model that is likely to have unstable homogeneous solutions. Consider
\[
W(E_1,E_2,G)=
\begin{cases}
\mathbb{W}(E_1,E_2,G,E_1E_2) & \text{if $E_1>0$ and $E_2>0$} \\
...
\end{equation}
we get the graphs of $F^*$ and $\lambda$ as functions of $a$ in Figure~\ref{fig:instability}. In particular notice that $\lambda$ became positive as $E_1^*$ is increased. This implies that, if $L$ is sufficiently large, then there exists $a$ such that $\lambda=\pi^2/L^2$.
\begin{rmk}
...
\begin{figure}
\begin{tikzpicture}
\begin{axis}[xlabel={$a$},ylabel={$F^*$},xmin=1,xmax=6,ymin=0,width=0.45\textwidth,ylabel style={rotate=-90}]
...
\end{axis}
\end{tikzpicture}
\caption{Graphs of $F^*$ and $\lambda$ as functions of the imposed elongation $a$ for the homogeneous deformation in the purely elastic model when $A=1$, $B=10^{-4}$, $p=2$, $\alpha=1$ and $r=2$. Notice that $\lambda$ became positive when $a$ is sufficiently large, indicating that the uniform solution is likely to became unstable if $L$ is large enough.}
