...
\end{itemize}
Assumption $(E)$ immediately implies that $\Psi$ is Fréchet differentiable with Fréchet differential $\partial\Psi(u)=\mathcal{A}$ for all $u\in V$. Since $\Phi$ is convex and Lipschitz continuous on its domain then it admits a non empty subdifferential $\partial\Phi(v)$ for all $v\in\dom\Phi$. \textbf{Per il teorem di Rockafeller sappiamo che è subdifferenziabile nell'interno del suo dominio, poi però data la Lipschitzianità su tutto il bordo immagino che si possa dimostrare che è subdifferenziabile su tutto il dominio in realtà.} In this case Problem~\ref{prb:modeling:doubly-nonlinear} can be formulated explicitely as follows.
\begin{prb}
\label{prb:evi}
...
The space $Z$ of the configurations $z=(u,p)$ will be clearly defined subsequently when the existence result will be proved. The virtual velocities will be denoted as $\tilde w=(\tilde v,\tilde q)$
...
The application of Theorem~\ref{thm:existence-energetic-solutions} leads to the desired existence result.
\begin{thm}
...
\begin{equation*}
y(t)\le C+\int_0^t\left(\alpha(s)y(s)+\beta(s)\right)ds\quad\text{for a.e. $t\in(0,T)$}.
\end{equation*}
Then
\begin{equation*}
...
\]
Moreover if $y_k,y\colon\Omega\to\mathbb{R}^n$ measurable satisfy
\[
y_k\to y\quad\text{in measure} 
\]
and $u_k,u\in L^1(\Omega,\mathbb{R}^m)$ satisfy
\[
u_k\to u\quad\text{in $L^1(\Omega,\mathbb{R}^n)$}
\]
then
\[
