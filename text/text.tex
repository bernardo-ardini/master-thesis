...
We start with the computation of the trivial solution of the form $\overline{y}_1(X,t)=\eta(t)X$, $\overline{y}_2(X,t)=\overline{y}_2(t)$ and $\overline P(X,t)=\overline{P}(t)$ for all $X\in(0,L)$ and $t\in[0,T]$. In the elastic range, $\overline y_2(t)$ can be found as we showed previously while $\overline P(t)=1$. When the Madel stress~\eqref{eqn:neck-large-mandel} overcomes the yield strength $Y$, then $\dot{\overline P}>0$ and the plastic force is everywhere given by $\partial_{\dot P}R(\overline P,\dot{\overline P})=Y/\overline P$. Then the balance of microforces coupled with the balance in the transversal direction gives a nonlinear algebraic equation that allows to compute $\overline y_2$ and $\overline P$.
...
\mathcal{E}(\overline{y}(t),\overline{P}(t),t)\le\mathcal{E}(\hat y,\hat P,t)+\mathcal{D}(\overline{P}(t),\hat P)\quad\text{for all $(\hat y,\hat P)\in\mathscr{Q}$}.
\end{equation}
is satisfied. Since $\mathcal{D}$ is a quasidistance, and in particular $\mathcal{D}(P_0,P_1)\ge0$ and equality holds if and only if $P_0=P_1$, the global stability condition is equivalent to say that $(\overline y(t),\overline P(t))$ is a global minimizer of $\mathcal{J}_t\colon\mathscr{Q}\to\mathbb{R}\cup\{\infty\}$ defined by
\[
\mathcal{J}_t(\hat y,\hat Q)=\mathcal{E}(\hat y,\hat P,t)+\mathcal{D}(\overline P(t),\hat P).
\]
...
\begin{rmk}
Our study of stability will be local, meaning that we will study the behavior of $\mathcal{J}_t$ near $(\overline{y}(t),\overline{P}(t))$ only, in particular taking into account second order variations. It is clear that if the trivial solution is locally stable, is does not necessarily global stable. Thus any estimate of the critical elongation corresponding with the onset of local instability must be intended as an upper bound of the actual critical elongation corresponding to global instability. In the case the actual critical elongation is smaller then the one predicted by the local analysis, the configuration of the bar would jump from the trivial configuration to another configuration energetically favored. However the experimental observation and the theoretical and numerical studies indicate in the introduction of Chapter~\ref{chap:intro-neck} suggest that necking happen without jumps. Thus we expect that our threshold of instability detected by the local analysis corresponds with the true instant at which necking starts. See also the discussion in Chapter~\ref{chap:solution-concepts} regarding the potential limitations of energetic solutions in identifying the exact onset of instability.
\end{rmk}
%The aim of what follows is to study the conditions under which there exists $t\in(0,T]$ such that~\eqref{eqn:stability-necking} is violated and understand qualitatively the behavior of the true stable solution. We will show that choosing suitably the constitutive relations, there exists $t_0\in(0,T]$ such that, if $t>t_0$ then~\eqref{eqn:stability-necking} is violated. To see this we will look at the behavior of $\mathcal{J}_t$ locally near $(\overline y(t),\overline P(t))$ studying its first and second variations on the directions in which it is smooth (notice indeed that $\mathcal{J}_t$ is non-smooth due to the non-smooth dissipation distance $\mathcal{D}$). It will result that the direction in which $\mathcal{J}_t$ decreases, preventing $(\overline y(t),\overline P(t))$ to be a minimizer, corresponds to an instability mode qualitatively similar to that we found in the purely elastic model and shown in Figure~\ref{fig:instability-mode-elastic}. This is interpreted as the onset of a neck in one of the extremities of the bar.
We start our analysis assuming that the stability condition is not violated in the elastic range. This is certainly true in the case of metals since yielding starts when the geometric nonlinearities have still a negligible effect. So fix $t\in[0,T]$ and suppose that the yield point has been already reached at that time. Given the difficulties in fully characterizing the exact stability condition~\eqref{eqn:stability-necking}, we deviate from it by considering two simplified conditions that can give us some intuition about the occurrence of instability.
...
\]
the reaction tensile force acting on the ends of the bar along the trivial solution. Consider now the function $J\colon(0,\infty)^3\to\mathbb{R}\cup\{\infty\}$ given by
\[
...
We say that the trivial uniform solution at time $t\in(0,T]$, is stable under force control if $(\eta(t),\overline{y}_2(t),\overline{P}(t))$ is a local minimizer of $J_t$.
\end{defi}
The mechanical meaning of this definition is the following: the trivial uniform solution $(\eta,\overline{y}_2(t),\overline{P}(t))$ is stable under force control when it is the most energetically favored configuration among all other uniform configurations and under the condition of fixed tensile force is applied. We now study whether the stability of the trivial uniform solution under force control is lost for sufficiently large elongations when the particular constitutive relations proposed in Section~\ref{subsec:constitutive-large-neck} are considered. In this particular case $J_t$ is not differentiable at $P=\overline{P}(t)$ but $J_t\in C^1(H^+_t)$ and $J_t\in C^1(H^-_t)$, where
\[
...
\]
Then, wether $(\eta(t),\overline y_2(t),\overline P(t))$ is a local minimum of $J_t$ depends on the eigenvalues of the hessian matrix
\[
h(t)=\nabla^2 J_t(\eta(t),\overline y_2(t),\overline P(t)^+).
\]
It is clear that at $t=0$ the trivial solution is stable and that $h(0)$ is positive definite. Then instability occur when $h(t)$ become singular. We now show that this happen exactly when $\overline{F}$ reaches its maximum, in agreement with Considère criterion. Indeed the following lemma holds.
\begin{lemma}
Let
\[
a(t)=h_{11}(t),\quad
...
Then it holds that
\[
\frac{d\overline F}{d\eta}(t):=\frac{d\overline F}{dt}(t)\left(\frac{d\eta}{dt}(t)\right)^{-1}=\frac{\det h(t)}{\det b(t)}.
\]
\end{lemma}
\begin{proof}
Notice that
\[
\overline F(s)=\dind{J_t}{\rho}(\eta(s),\overline y_2(s),\overline P(s))+\overline{F}(t).
\]
Then, differentiating with respect to $s$, computing for $s=t$ and dividing by $\frac{d\eta}{dt}(t)$, we get
\begin{equation}
\label{eqn:dF-deta-1}
\frac{d\overline F}{d\eta}=\dind{J_t}{\rho\partial\rho}(\eta,\overline y_2,\overline P)+\dind{J_t}{\rho\partial y_2}(\eta,\overline y_2,\overline P)\frac{d\overline y_2}{d\eta}+\dind{J_t}{\rho\partial P}(\eta,\overline y_2,\overline P)\frac{d\overline P}{d\eta}.
\end{equation}
...
\[
\frac{d\overline F}{d\eta}=a-c\cdot b^{-1}c.
\]
But since
\[
...
\end{axis}
\end{tikzpicture}
\caption{Graphs of $\overline F$ and $\det h$ as functions of the elongation $\eta$ along the trivial solution in the plastic model. Notice that the maximum of $\overline{F}$ is reached when $h$ vanishes at $\eta=1.1474$.}
\label{fig:instabilitypl}
\end{figure}

\subsubsection*{Regularized stability}

The feature that makes inequality~\eqref{eqn:stability-necking}, or equivalently the minimality of $\mathcal{J}_t$ at $(\overline{y}(t),\overline{P}(t))$ difficult to analyze, is the lack of smoothness of $\mathcal{D}$. This indeed prevents us from considering the first and second variation of $\mathcal{J}_t$ to check whether $(\overline{y}(t),\overline{P}(t))$ is a (local) minima. The idea to circumvent these difficulties would be to consider variations in the directions where the directional derivatives exists. Specifically we restrict to the particular constitutive relation for the dissipation potential discussed in Section~\ref{subsec:constitutive-large-neck}, we define the two smooth functionals $\mathcal{J}_t^\varsigma\colon\mathscr{Q}\to\mathbb{R}\cup\{\infty\}$ for $\varsigma\in\{\pm1\}$ defined by
\[
\mathcal{J}_t^\varsigma(\hat y,\hat P,t)=\int_0^LW(\eta(t)\hat y_1'\hat P^{-1},\hat y_2\hat P,\hat y_2'\hat P^{-1})dX+\int_0^LH(\hat P,\hat P')dX+\int_0^LD^\varsigma(\overline P,\hat P)dt,
\]
...
\[
\mathscr{Q}^\varsigma_t=\{\hat q=(\hat y,\hat P)\in\mathscr{Q}\;|\;\text{$\varsigma(\hat P-\overline P(t))\ge0$ in $(0,L)$}\}.
\]
...
\begin{equation}
\label{eqn:min-neck}
\mathcal{J}_t(\overline{q}(t))\le\mathcal{J}_t(\hat q)\quad\text{for all $\hat q\in\mathscr{Q}$}
\end{equation}
implies
\begin{equation}
\label{eqn:min-neck-sigma}
\mathcal{J}_t^\varsigma(\overline{q}(t))\le\mathcal{J}_t^\varsigma(\hat q)\quad\text{for all $\hat q\in\mathscr{Q}^\varsigma_t$ and $\varsigma\in\{\pm1\}$}.
\end{equation}
\end{lemma}
%\begin{lemma}
%The stability condition
...
%\end{lemma}
%\begin{proof}
%That~\eqref{eqn:min-neck} implies~\eqref{eqn:min-neck-sigma} is trivial. Conversely suppose that~\eqref{eqn:min-neck-sigma} holds and, by contradiction, that $\mathcal{J}_t(\overline{q}(t))>\mathcal{J}_t(\hat q)$ for some $\hat q\in\mathscr{Q}$. If $\hat q\in\mathscr{Q}^\varsigma$ for some $\varsigma\in\{\pm1\}$ we immediately get the desired contradiction. Otherwise let $\hat q^+\in\mathscr{Q}^+$ be defined by $\hat y^+=\hat y$ and $\hat P^+:=\max\{\hat P,\overline P(t)\}$. Then
%\[
%\mathcal{J}_t^+(\overline{q}(t))=\mathcal{J}_t(\overline{q}(t))>\mathcal{J}_t(\hat q)\ge\mathcal{J}_t^+(\hat q^+).
%\]
...
Considering the way in which the trivial solution is computed (see the beginning of Section`\ref{subsec:plastic-model}) it is immediate to check that $\partial\mathcal{J}^+_t(\overline q(t))=0$ for all $t\in[0,T]$. This implies that for all $w=(v,Q)\in Q$ with $Q\le0$ in $(0,L)$, it holds that
\[
\dual{\partial\mathcal{J}^-_t(\overline q(t)),w}=\dual{\partial\mathcal{J}^+_t(\overline q(t)),w}-2Y\int_0^L\log\frac{Q}{\overline P(t)}dX=-2Y\int_0^L\frac{Q}{\overline P(t)}dX\ge0.
\]
This means that~\eqref{eqn:min-neck-sigma} for $\varsigma=-1$ is satisfied, at least locally for $\hat q$ near $\overline q(t)$, for all $t\in[0,T]$. So we turn to study whether~\eqref{eqn:min-neck-sigma}, for $\varsigma=1$, is violated. To this aim we could study the second variation of $\mathcal{J}_t^+$ at $(\overline{y}(t),\overline P(t))$: if we are able to find $w=(v,Q)\in Q$ with $Q\ge0$ in $(0,L)$ such that $\dual{\partial^2\mathcal{J}_t^+(\overline q(t))w,w}<0$, we have done. Clearly the critical condition would be the existence of a nontrivial $w=(y,Q)$ with $Q\ge0$ such that $\partial^2\mathcal{J}_t^+(\overline q(t))w=0$. This last equation is the linerization of the perturbed equation $\partial\mathcal{J}_t^+(\overline q(t)+w)=0$. Since our efforts to find a nontrivial solution $w$ satisfying the constraint $Q\ge0$ were unsuccessful, in what follows we presents a simplified analysis in which that constraint is disregarded. This leads us to the following second simplified notion of stability.
\begin{defi}[regularized stability]
We say that the trivial uniform solution satisfies the regularized stability condition if $\partial^2\mathcal{J}_t^+(\overline y(t),\overline P(t))$ is positive definite.
\end{defi}
So, now we study the perturbed equation equation $\partial^2\mathcal{J}_t^+(\overline q(t))w=0$. After some tedious computation we wind that (for each $t$ fixed that we will not specify anymore) this is equivalent to
\begin{equation}
\label{eqn:2order}
...
\[
w_1=\sum_{k=1}^\infty w_1^k\sin\left(\frac{k\pi}{L}x\right),\quad w_2=w_2^0+\sum_{k=1}^\infty w_2^k\cos\left(\frac{k\pi}{L}x\right)\quad\text{and}\quad w_3=w_3^0+\sum_{k=1}^\infty w_3^k\cos\left(\frac{k\pi}{L}x\right).
\]
Substituting into~\eqref{eqn:equations2order} we get
\begin{gather}
...
\end{gather}
in $(0,L)$. Taking into account the orthogonality relations
\[
\int_0^L\sin(\gamma_kx)\sin(\gamma_jx)dx=\begin{cases}
\frac{L}{2} & \text{if $k=j$} \\
0 & \text{if $k\neq j$}
\end{cases}\quad\text{and}\quad\int_0^L\sin(\gamma_kx)\sin(\gamma_jx)dx=\begin{cases}
L & \text{if $k=$ and $j=0$}
\end{cases}\frac{L}{2}\delta_{kj}
\]
...
\]
where $\beta>0$ and $\Theta>0$. Define  $f$ and $g$ as the signed distance from $C$ and $G$ respectively, and let $\hat{R}$ defined as in~\eqref{eqn:R-hat}. Let $\overline u\in H^1((0,T),H^1(\Omega,\mathbb{R}^d))$ and $(u_0,E_p^0)\in H^1(\Omega,\mathbb{R}^d)\times L^2(\Omega,\text{Sym})$ such that $u_0=\overline{u}(0)$ in $\Gamma_D$ in the sense of the trace. Then there exists a unique couple $(u,E_p)$ with
\[
...
\end{thm}
\begin{rmk}
Using a time rescaling approach it is possible to prove also the existence of a limit solution $(u,E_p)$ in the limit $\eta\to0^+$ of vanishing viscosity, with $u\in C^0([0,T],\text{BD}(\Omega))$ and $E_p\in W^{1,\infty}((0,T),\mathscr{M}(\overline\Omega,\text{Sym}))$, where $\text{BD}(\Omega)$ is the space of functions of bounded variations and $\mathscr{M}(\overline\Omega,\text{Sym})$ the space of bounded Radon measures.
\end{rmk}
