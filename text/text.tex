...
Let $Y=W^{1,q_Y}((0,L),\mathbb{R}^2)$ with $q_Y\in(??,\infty)$ and $Z=L^{q_H}((0,L))\cap W^{1,r}((0,L))$. Set $Q=Y\times Z$. PER ORA IL VINCOLO SUGLI ESPONENTI È CHE DEVONO ESSERE TUTTI MAGGIORI DI  E MINORI DI INFINITO E $\frac{1}{q_Y}+\frac{1}{q_H}<1$. Some times the notation $G=E_3$ and $E=(E_1,E_2,E_3)$ will be used.
...
\item[(W4)] $W$ is differentiable in $\dom W$ and there exist $C^W_0\in\mathbb{R}$ and $C^W_1>0$ such that for all $E\in\dom W$ and $i\in\{1,2,3\}$,
\begin{equation*}
\left|E_i\dind{W}{E_i}(E)\right|\le C_1^W(C_0^W+W(E)).
\end{equation*} 
\end{itemize}
...
We assume that
\[
\frac{1}{q_Y}=\frac{1}{q_W}+\frac{1}{q_H}.
\]
\begin{prop}[Coercivity of the energy]
The sublevels of $\mathcal{E}$ are bounded in $Q$.
\end{prop}
\begin{proof}
...
\begin{prop}[convergence of elastic strain measures]
\label{prop:convergence-minors}
Let $q_Y,s\in(1,\infty)$ such that $\frac{1}{q}+\frac{1}{s}\le1$. Let $y^k,y\in W^{1,q}((0,L),\mathbb{R}\times\mathbb{R})$ and $P_k,P\in L^s((0,L))$ such that
\begin{gather*}
y^k\rightharpoonup y\quad\text{in $W^{1,q_Y}((0,L))$} \\
P_k\to P\quad\text{in $L^s((0,L))$}.
\end{gather*}
...
\end{prop}
\begin{proof}
First notice that $P_k^{-1}\to P^{-1}$ in $L^s((0,L))$. Indeed
\[
...
Let $q,s\in(1,\infty)$ such that $\frac{1}{q}+\frac{1}{s}\le1$. Let $f_k,f\in L^q((0,L))$, $g_k,g\in L^q((0,L))$, $h_k,h\in L^s((0,L))$ such that
\begin{gather*}
f_k\rightharpoonup f\quad\text{in $L^q((0,L))$} \\
g_k\to g\quad\text{in $L^\infty((0,L))$} \\
h_k\to h\quad\text{in $L^s((0,L))$}.
\end{gather*}
...
\end{multline*}
Since $f_k$ are bounded in $L^q((0,L))$, $h_k$ are bounded in $L^s((0,L))$ and $\frac{1}{q}+\frac{1}{s}\le1$, then $f_kh_k\phi$ are bounded in $L^1((0,L))$ and $gf_k\phi$ are bounded in $L^{s^*}((0,L))$. Moreover $gh\phi\in L^{q^*}((0,L))$. Then the thesis follows.
\end{proof}
...
\[
\mathcal{E}(y,P)\le\liminf_{k\to\infty}\mathcal{E}(y_k,P_k).
\]
We take a (not relabeled) subsequence such that $\mathcal{E}(y_k,P_k)$ has a limit. Then we need to show that
\begin{equation}
\label{eqn:lower-semicontinuity-E-limit}
\mathcal{E}(y,P)\le\lim_{k\to\infty}\mathcal{E}(y_k,P_k).
\end{equation}
If $\mathcal{E}(y_k,P_k)\to\infty$ we have nothing to prove. Then we assume that instead it converges to a finite limit. In this case we can assume that $\mathcal{E}(y_k,P_k)<\infty$ for all $k$. This implies that $P_k\ge p$ a.e.\ in $(0,L)$. In particular also $P\ge c$ a.e.\ in $(0,L)$.
...
\[
|\partial_t\mathcal{E}(q,t)-\partial_t\mathcal{E}(q,s)|\le\omega^E(|t-s|)(C_0^E+\mathcal{E}(q,t)).
\]
\end{itemize}
\end{prop}
\begin{proof}
...
\]
Let $E_1=g_1'(y_1,t)y_1'P^{-1}$, $E_2=g_2(y_1,t)y_2P$ and $G=g_2(y_1,t)y_2'P^{-1}$ as functions of $(y,y',P,t)\in\mathbb{R}^2\times\mathbb{R}^2\times\mathbb{R}\times t\in[0,T]$. Let $q=(y,P)\in\mathscr{D}$. Then $(E_1,E_2,G)\in\dom W$ a.e.\ on $(0,L)$ and everywhere on $[0,T]$. Then $t\mapsto W(E_1,E_2,G)$ is differentiable in $[0,T]$ and a.e.\ in $(0,L)$ and
\[
...
\]
there. We first prove that for all $t\in[0,T]$, there exists
\begin{equation}
...
Taking into account the fact that $\frac{\dot{g}'_1}{g'_1}$ and $\frac{\dot{g}_2}{g_2}$ are bounded in $\mathbb{R}\times[0,T]$, assumption (W4) yields
\[
...
\end{lemma}
\begin{proof}
\emph{Step 1.} For all $i\in\{1,2,3\}$,
\[
\begin{split}
...
\appendix
\part*{Collection of known results}
...
Let
\begin{equation}
\parbox[c]{0.8\textwidth}{$Y$ and $Z$ separable and reflexive Banach spaces}.
\tag{S1}
\end{equation}
and
\begin{equation}
\parbox[c]{0.8\textwidth}{$\mathscr{Y}$ and $\mathscr{Z}$ weakly closed subsets of $Y$ and $Z$ respectively}.
\tag{S1}
\end{equation}
Set $Q=Y\times Z$ and $\mathscr{Q}=\mathscr{Y}\times\mathscr{Z}$. The assumptions on the energy functional are that
\begin{gather}
\label{eqn:ES-energy-definition}
\parbox[c]{0.8\textwidth}{$\mathcal{E}\colon[0,T]\times\mathscr{Q}\to(-\infty,\infty]$}
\tag{E0}
\\
\label{eqn:ES-energy-coercivity}
\parbox[c]{0.8\textwidth}{for all $t\in[0,T]$ and $E\in\mathbb{R}$ the set $\mathscr{D}_E:=\{q\in\mathscr{Q}\;|\;\mathcal{E}(q,t)\le E\}$ is bounded and weakly closed}
\tag{E1}
\\
\label{eqn:ES-energy-space-regularity}
\parbox[c]{0.8\textwidth}{there exists $\mathscr{D}\subset\mathscr{Q}$ such that $\dom\mathcal{E}(\cdot,t)=\mathscr{D}$ for all $t\in[0,T]$}
\tag{E2}
\\
\label{eqn:ES-energy-time-regularity}
\parbox[c]{0.8\textwidth}{for all $q\in\mathscr{D}$, the function $\mathcal{E}(q,\cdot)$ is $C^1([0,T])$ and
\begin{itemize}
...
\item[(ii)] there exists a modulus of continuity $\omega^E$ such that for all $q\in\mathscr{D}$ and $t,s\in[0,T]$
\[
|\partial_t\mathcal{E}(q,t)-\partial_t\mathcal{E}(q,s)|\le\omega(|t-s|)(C_0^E+\mathcal{E}(q,t)).
\]
\end{itemize}
}
\tag{E3}
\end{gather}
Notice that~\eqref{eqn:ES-energy-space-regularity} implies that $\mathcal{E}$ is weakly lower semicontinuous and that~\eqref{eqn:ES-energy-time-regularity} implies the existence of a modulus of continuity $\omega^E$ (this means $\omega^E\colon[0,\infty)\to[0,\infty)$ is continuous, nondecreasing and $\omega^E(0)=0$) such that
\[
|\partial_t\mathcal{E}(q,t_1)-\partial_t\mathcal{E}(q,t_0)|\le\omega^E(|t_1-t_0|)\quad\text{for all $q\in\mathscr{D}$ and $t_0,t_1\in[0,T]$.}
\]
The bound in~\eqref{eqn:ES-energy-time-regularity} allows to get the bound
\begin{equation}
...
\begin{gather}
\label{eqn:ES-dissipation-definition}
\parbox[c]{0.8\textwidth}{$\mathcal{D}\colon\mathscr{Z}\times\mathscr{Z}\to[0,\infty]$ is a quasidistance, that is $\mathcal{D}(z_0,z_1)=0$ if and only if $z_0=z_1$ and $\mathcal{D}(z_0,z_2)\le\mathcal{D}(z_0,z_1)+\mathcal{D}(z_1,z_2)$ for all $z_1,z_2\in\mathscr{Z}$}
\tag{D1}
\\
\label{eqn:ES-dissipation-quasi-distance}
\parbox[c]{0.8\textwidth}{$\mathcal{D}$ is weakly lower semicontinuous, meaning that $z_n\rightharpoonup z$ and $\hat{z}_n\rightharpoonup\hat{z}$ implies $\mathcal{D}(z,\hat{z})\le\liminf_{n\to\infty}\mathcal{D}(z_n,\hat{z}_n)$.}
\tag{D2}
\end{gather}
...
\begin{gather}
\label{eqn:ES-compatibility-1}
\parbox[c]{0.8\textwidth}{$\partial_t\mathcal{E}$ is continuous along stable sequences, this means that for all stable sequences $(q_n,t_n)\in\mathscr{Q}\times[0,T]$ such that $q_n\rightharpoonup q$ and $t_n\to t$, it holds that $\partial_t\mathcal{E}(q_n,t)\to\partial_t\mathcal{E}(q,t)$}
\tag{C1}
\\
\label{eqn:ES-compatibility-2}
\parbox[c]{0.8\textwidth}{the stable sets are closed in the sense that, if $(q_n,t_n)\in\mathscr{Q}\times[0,T]$ is a stable sequence such that $q_n\rightharpoonup q$ and $t_n\to t$, then $q\in\mathscr{S}(t)$}
\tag{C2}
\end{gather}
