...
\[
W(E_1,E_2,G)=C\left(\frac{E_1^2+l^2G^2+E_2^2}{E_1E_2}-2\right)+w(E_1E_2).
\]
In this case
\begin{gather*}
\sigma_1=C\left(\frac{1}{E_2}-\frac{l^2G^2}{E_1^2E_2}-\frac{E_2}{E_1^2}\right)+E_2w'(E_1E_2),\quad\sigma_2=C\left(-\frac{E_1}{E_2^2}-\frac{l^2G^2}{E_1E_2^2}+\frac{1}{E_1}\right)+E_1w'(E_1E_2) \\
\zeta=2C\frac{l^2G}{E_1E_2}
\end{gather*}
...
Let $Y=W^{1,q_Y}((0,L),\mathbb{R}^2)$ with $q_Y\in(1,\infty)$ the space of $y$ and $Z=L^{q_H}((0,L))\cap W^{1,r}((0,L))$ with $q_h,r\in(1,\infty)$ the space of $P$. Set $Q=Y\times Z$.
...
\begin{itemize}[itemsep=1em]
\item[(W1)] there exists $\mathbb{W}\colon\mathbb{R}^4\to(-\infty,\infty]$ continuous and convex such that $W(E_1,E_2,G)=\mathbb{W}(E_1,E_2,G,E_1E_2)$ for all $E_1,E_2,G\in\mathbb{R}$,
\item[(W2)] there exist $C_W>0$ and $q_W\in(1,\infty)$ such that $W(E_1,E_2,G)\ge C_W(-1+|E_1|^{q_F}+|E_2|^{q_F}+|G|^{q_F})$ for all $E_1,E_2,G\in\mathbb{R}$,
...
\item[(H2)] there exist $C_H>0$ and $q_H,r\in(1,\infty)$ such that $H(P,P')\ge C_H(-1+|P|^{q_H}+|P'|^r)$ for all $P,P'\in\mathbb{R}$,
...
\begin{itemize}[itemsep=1em]
\item[(d1)] $D\colon\mathbb{R}^2\to[0,\infty]$ is continuous,
\item[(d2)] there exist $C_D>0$ and $q_D\in[1,q_H)$ such that $D(P_0,P_1)\le C_D(1+|P_0|^{q_D}+|P_1|^{q_D})$ for all $P_1,P_2\in\mathbb{R}$,
\item[(d3)] $D$ is a quasidistance, in the sense that $D(P_0,P_1)=0$ if and only if $P_0=P_1$ and $D(P_0,P_2)\le D(P_0,P_1)+D(P_1,P_2)$ for all $P_0,P_1,P_2\in\mathbb{R}$.
\end{itemize}
...
\end{split}
\]
Since $L^\infty((0,T))$ is continuously embedded in $W^{1,q}((0,L))$, it follows that for all $\epsilon>0$ there exists $C>0$ such that
\[
\norm{y_2}_\infty\le C(\norm{y_2}_q+\epsilon\norm{y_2'}_q)\quad\text{for all $y_2\in W^{1,q}((0,L))$}.
\]
...
Let $y^k,y\in W^{1,q}((0,L),\mathbb{R}\times\mathbb{R})$ and $P_k,P\in C^0([0,L])$ such that
\begin{gather*}
...
Let $f_k,f\in L^q((0,L))$, $g_k,g\in L^\infty((0,L))$ with $q\in(1,\infty)$ and $h_k,h\in C^0([0,L])$ such that
\begin{gather*}
...
\end{align*}
By these convergence results, together with $P_k\to P$ in $C^0([0,L])$ and $P_k'\rightharpoonup P'$ in $L^r((0,T))$, and by the convexity assumptions (W0) and (H0), we can apply Theorem~\ref{thm:lower-semicontinuity-integral-functional} which leads to~\eqref{eqn:lower-semicontinuity-E-limit}. This concludes the proof.
\end{proof}
...
Notice that (d1) implies that $\mathcal{D}$ is well defined. Property (D1) follows immediately form (d3). We now prove (D2).
\begin{prop}
...
\]
Since $P_k$ and $\hat{P}_k$ are bounded in $W^{1,r}((0,L))$, for each subsequence $P_{k_j}$ and $\hat{P}_{k_j}$, up to extracting a further subsequence,
\[
P_{k_j}\to P\quad\text{and}\quad\hat{P}_{k_j}\to\hat{P}\quad\text{$C^0([0,L])$.}
\]
Then actually the whole sequence converges uniformly. i.e.,
\[
...
\[
\liminf_{k\to\infty}\mathcal{D}(P_k,\hat{P}_k)\ge\int_0^L\liminf_{k\to\infty}D(P_k,\hat{P}_k)dX=\int_0^L D(P,\hat{P})dX.
\]
The last equality holds since $D$ is continuous. This concludes the proof.
\end{proof}

\subsection{Verification of the compatibility conditions}

We now verify (C1) and (C2). In view of Proposition~\ref{prop:tool-verify-compatibility} we need to verify (PC1) and (PC2). Notice that (PC2) has been established in Proposition~\ref{prop:power-control-necking} (iii). This we need to verify (PC1) only.
...
\end{itemize}
\end{itemize}
Notice that~(E2) implies that $\mathcal{E}$ is weakly lower semicontinuous. Moreover~(E4, i) implies
\begin{equation}
...
\begin{itemize}[itemsep=1em]
\item[(C1)] $\partial_t\mathcal{E}$ is continuous along stable sequences, this means that for all stable sequences $(q_n,t_n)\in\mathscr{Q}\times[0,T]$ such that $q_n\rightharpoonup q$ and $t_n\to t$, it holds that $\partial_t\mathcal{E}(q_n,t)\to\partial_t\mathcal{E}(q,t)$,
