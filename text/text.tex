...
\caption{Instability mode of the elastic bar under traction.}
...
\[
J(\rho,y_2,P)=W(\rho P^{-1},y_2P,0)+H(P,0)+D(\overline{P}(t),P)-\overline{F}(t)\rho.
\]
The quantity $J(\rho,y_2,P)-J(\eta(t),\overline{y}_2(t),\overline{P}(t))$ is the energy per unit length that must be supplied to the system to bring it from the configuration $\overline y_1(t)$, $\overline y_2(t)$ and $\overline P(t)$ to the configuration $y_1(X)=\rho X$, $y_2(X)=y_2$ and $P(X)=P$ if the ends are now left free to move and the tensile force is kept equal to $\overline F(t)$. We now want to study if $(\eta,\overline{y}_2(t),\overline{P}(t))$ is a (local) minimizer of $J$. If not, this would signify that the trivial solution ($\overline y,\overline P$) is not the most energetically favored it the same tensile force is applied and the ends are made free to move. Thus assume now the particular constitutive relations proposed in Section~\ref{subsec:constitutive-large-neck}. In this case $J$ is not differentiable at $P=\overline{P}(t)$ but $J\in C^1(H^+)$ and $J\in C^1(H^-)$, where
\[
H^+=\{(\rho,y_2,P)\in(0,\infty)^3\;|\;P\ge\overline{P}(t)\}\quad\text{and}\quad H^-=\{(\rho,y_2,P)\in(0,\infty)^3\;|\;\Pi<P\le\overline{P}(t)\}.
\]
We easily find that
\[
\nabla J(\eta,\overline y_2(t),\overline P(t)^-)=\left(0,0,-\frac{2Y}{\overline P(t)}\right)\quad\text{and}\quad\nabla J(\eta,\overline y_2(t),\overline P(t)^+)=(0,0,0).
\]
Then, to study wether $(\eta(t),\overline y_2(t),\overline P(t))$ is a local minimum look at the hessian matrix
\[
\nabla^2 J(\eta(t),\overline y_2(t),\overline P(t)^+)=
\]

Consider the elastic free energy density proposed previously for the purely elastic model. Consider also the following hardening free energy densities,
\begin{itemize}
\item[(i)] a simple quadratic energy density, with an additional penalization as $P$ approaches to $0<p<1$,
\[
H(P,P')=
\begin{cases}
C(P-1)^2+\frac{D}{P-p}+E|lP'|^q & \text{if $P>p$} \\
\infty & \text{otherwise},
\end{cases}
\]
\item[(ii)] and a more refined model
\[
H(P,P')=
\begin{cases}
C\left(P^q+\frac{q}{\delta}e^{-\delta(P-1)}\right)+\frac{D}{P-p}+E|lP'|^q & \text{if $P>p$} \\
\infty & \text{otherwise},
\end{cases}
\]
\end{itemize}
...
\begin{rmk}
The expression of $H$ in (ii) is obtained as a modification of the Armstrong--Frederick kinematical hardening model. According to the Armstrong--Frederick model, the backstress $\beta=\dind{H}{P}$ satisfies
\begin{equation}
...
\]
and formula in (ii) is a modification of this as the linear growth in $P$ has been substituted with a superlinear growth in order to satisfy (H2). The superlinear growth makes $\beta$ unbounded as $\beta\to\infty$, but the main features of the original Armstrong--Frederick model are preserved. \textbf{Ma poi per garantire che i sottolivelli siano limitati, non andebbe a limite anche bene che $H$ abbia crescita solo lineare in $P$? Tanto poi il termine col gradiente compensa questa cosa.} In particular $H$ in (ii) is non-convex in $P$.
\end{rmk}
Let also
\[
R(P,\dot P)=
\begin{cases}
Y\frac{|\dot P|}{P} & \text{if $P\ge p$} \\
\infty & \text{otherwise},
\end{cases}
\]
with $Y>0$. In Remark~\ref{rmk:dissipation-distance-density} we already showed that the corresponding dissipation distance density $D\colon\mathbb{R}\to[0,\infty]$,
\[
D(P_0,P_1)=
\begin{cases}
Y|\log P_1-\log P_0| & \text{if $P_0,P_1\ge p$} \\
...
In Figure~\ref{fig:instabilitypl} is reported the graph of the tensile force $F^*$ along the homogeneous solution as a function of $a$. The solid line correspond to $H$ in (ii) while the dashed line to $H$ in (i).
...
\end{axis}
\end{tikzpicture}
\caption{Graphs of $F^*$ and $\lambda$ as functions of the elongation $a$ along the homogeneous solution in the plastic model. The solid line is referred to $H$ in (ii) and the dashed line to $H$ in (i).}
