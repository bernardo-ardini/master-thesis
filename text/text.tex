...
\usepackage{geometry}
\geometry{a4paper,margin=3cm,heightrounded}
...
In this section we describe a general framework that allows to systematically develop mechanical models that are consistent with thermodynamics. All the models presented in this thesis will be formulated in agreement with this framework. What follows is based on the examples provided by~\cite{gurtin-anand-fried},\cite{gurtin-anand-small} and~\cite{gurtin-anand-large}.

\subsection{Kinematics}

Kinematics is concerned with the description of the \emph{configurations} that a mechanical systems can assume. We assume that the possible configurations can be described as the elements of a Banach space $V$.
\begin{ex}
...
Consider a body that can be subject to damage. This can be modeled considering an internal damage variable $z\in W^{1,p}(\Omega)$ with $p\in(0,1)$. Where $z=1$ the material is sound, while where $z=0$ the material is completely damaged. The material is partially damaged where $z\in(0,1)$. One should add the constrain $z\in[0,1]$, but this is not required in the definition of the space of configurations in order to preserve the linear structure. As in Example~\ref{ex:modeling:large-strain}, that constrain can be imposed later.
\end{ex}
\begin{rmk}
Examples~\ref{ex:modeling:large-strain} and~\ref{ex:modeling:micro} shows that the assumption that space of configurations has a linear structure is not restrictive in most mechanical models, even when the attainable configurations actually lie in a space $M$ with a manifold structure. Indeed in most cases, $M$ can be seen as a subset of a larger linear space of configurations $V$. Thus, the complexity of the geometry of $M$ can be reduced to the analytical complexity of managing the singular reactions required to ensure that the configurations belong to $M$.
\end{rmk}
\begin{rmk}
As Example~\ref{ex:modeling:micro} demonstrate, the mechanical framework we are developing includes also models where internal variables are added to characterize microscopic processes. 
\end{rmk}
...
\chapter{Model in regime of small deformations}
...
We denote with $\tilde v$ and $\tilde q$ the virtual rates of $u$ and $p$ respectively. To describe the coupling between the macroscopic configuration $u$ and the microscopic configuration $p$ we choose an internal generalized force of the form
\[
\tilde{\mathcal I}=\int_0^L\zeta\tilde v_2'dx+\int_0^L\left(\sigma_1(\tilde v_1'-\tilde q)+\sigma_2(\tilde v_2+(d-1)^{-1}\tilde q)\right)dx+\int_0^L\tau\tilde qdx.
\]
The first integral describes a purely macroscopic response.. Here $\zeta$ is the transversal stress. The second integral describes the elastic response that couples the macroscopic and the microscopic configurations. Here $\sigma_1$ is the longitudinal elastic stress and $\sigma_2$ the longitudinal elastic force. The last integral describes the microscopic response and $\tau$ is the microscopic force. The external longitudinal and transversal forces distributed along the length of the bar and the external longitudinal and transversal forces acting on the extremities of the bar are included choosing the external generalized force
\[
...
\chapter{Model in regime of large deformations}
...
\end{pmatrix}
\end{equation}
where $P\colon(0,L)\to(0,\infty)$. This choice implies plastic incompressibility. We define $p=\log P$. The elastic distortion is then given by the Kroner--Lee multiplicative decomposition
\[
...
\[
\tilde{\mathcal{I}}=\int_0^L\left(\zeta\tilde{\dot G}+\sigma_1\tilde{\dot E}_1+\sigma_2\tilde{\dot E}_2\right)dX+\int_0^L\left(\tau\tilde{\dot P}+\kappa\tilde{\dot P}'\right)dX,
\]
where the tilde symbol denotes the virtual rates. Here $\zeta$ is the transversal elastic stress, $\sigma_1$ is the longitudinal elastic stress and $\sigma_2$ is the transversal elastic force while $\tau$ is the microscopic force and $\kappa$ the microscopic stress.
\begin{rmk}
...
\end{gather*}
where $\tilde v$ and $\tilde Q$ are the virtual rates of $\chi$ and $P$ respectively. Then the internal generalized force can be written more explicitly as
\begin{multline*}
\tilde{\mathcal{I}}=\int_0^LG\zeta\left((\chi_2')^{-1}\tilde v_2'-P^{-1}\tilde Q\right)dX+\\
...
\end{multline*}
The external longitudinal and transversal forces distributed along the length of the bar and the external longitudinal and transversal forces acting on the extremities of the bar are included choosing the external generalized force
\[
\tilde{\mathcal X}=\int_0^Lf\cdot\tilde vdx+h(L)\cdot\tilde v(L)-h(0)\cdot\tilde v(0).
\]
\begin{prop}
...
\subsubsection*{Particular constitutive relations} Particular constitutive relations are the following:
\begin{gather}
W(E_1,E_2,G)=
\begin{cases}
\alpha_1|E_1|^{q_W}+\alpha_2|E_2|^{q_W}+\alpha_3 l^2|G|^{q_W}+\alpha_4\frac{1}{|E_1E_2|^{s_W}} & \text{of $E_1,E_2>0$} \\
\infty & \text{otherwise}
\end{cases}
\\
H(P,P')=
\begin{cases}
\alpha_5|P|^{q_H}+\alpha_6\frac{1}{|P-p|^{s_H}}+\alpha_7|P'|^{r_H} & \text{if $P>p$} \\
\infty & \text{otherwise}
\end{cases}
\\
R(P,\dot P)=
\begin{cases}
\alpha_8\frac{|\dot P|}{P} & \text{if $P\ge p$} \\
\infty & \text{otherwise},
\end{cases}
\end{gather}
where $\alpha_1,\dots,\alpha_8>0$, $s_W,s_H\in(0,\infty)$, $q_W,q_H,r_H\in(1,\infty)$ and $0<p<1$. This constitutive relations are motivated by the existence theorem that we prove in Section~\ref{sec:existence-necking}.
\begin{rmk}
\label{rmk:dissipation-distance-density}
Let the dissipation distance density $D\colon\mathbb{R}\to[0,\infty]$ be defined by
\[
...
D(P_0,P_1)=
\begin{cases}
\alpha_8|\log P_1-\log P_0| & \text{if $P_0,P_1\ge p$} \\
\infty & \text{otherwise}.
\end{cases}
\]
Indeed notice that for each $P\in C^1([0,1],(0,\infty))$ connecting $P_0$ with $P_0$, it holds
\[
\int_0^1R(P(t),\dot{P}(t))dt\ge\alpha_8\int_0^1\left|\frac{\dot{P}(t)}{P(t)}\right|dt\ge\alpha_8\left|\int_0^1\frac{\dot{P}(t)}{P(t)}dt\right|=\alpha_8|\log P_1-\log P_0|.
\]
This implies $D(P_0,P_1)\ge\alpha_8|\log P_1-\log P_0|$. If $P_i<p$ for some $i\in\{0,1\}$, then every $C^1$ curve $P$ connecting $P_0$ with $P_1$, has $\int_0^1R(P(t),\dot{P}(t))dt=\infty$. If instead $P_0,P_1\ge p$ then we can take $P(t)=P_0^tP_1^{1-t}\ge p$ and in this case
\[
\int_0^1R(P(t),\dot{P}(t))dt=\alpha_8\int_0^1\left|\frac{\dot{P}(t)}{P(t)}\right|dt=\alpha_8|\log P_1-\log P_1|.
\]
...
\end{multline*}
The dissipation distance can be expressed in term of the dissipation distance density $D\colon\mathbb{R}^2\to[0,\infty]$, discussed in Remark~\ref{rmk:dissipation-distance-density}, as
\[
...
\item[(H3)] there exists $p>0$ such that $H(P,P')=\infty$ if and only if $P\le p$.
\end{itemize}
...
\item[(d3)] $D$ is finite and continuous on $[p,\infty)\times[p,\infty)$, where $p$ is the constant appearing in (H3).
\end{itemize}
...
\end{gather*}
Assume also that $P_k,P\ge p>0$ a.e.\ in $(0,L)$. Let also $\gamma_1^k,\gamma_1,\gamma_2^k,\gamma_2,\rho_2^k,\rho_2\in C^0([0,L])$ such that
\begin{align*}
...
\[
|P_k^{-1}-P^{-1}|=\frac{|P_k-P|}{|P_kP|}\le\frac{|P_k-P|}{p^2}.
\]
...
\end{equation}
If $\mathcal{E}(y_k,P_k,t)\to\infty$ we have nothing to prove. Then we assume that instead it converges to a finite limit. In this case we can assume that $\mathcal{E}(y_k,P_k,t)<\infty$ for all $k$. This implies that $P_k>p$ a.e.\ in $(0,L)$, so that also $P\ge p$ a.e.\ in $(0,L)$.
...
\]
Since $\hat{q}\in\mathscr{D}$, then (H3) implies that $\hat{P}>p$ a.e.\ in $(0,L)$, so that $\hat{P}\ge p$ in $[0,L]$. Since also $q_k\in\mathscr{D}$ then, for the same reason, also $P_k\ge p$ in $[0,L]$. Notice also that by~\ref{prop:sobolev-weak-lp-strong}, $P_k\to P$ in $C^0([0,T])$. Then actually
\[
\lim_{k\to\infty}\mathcal{D}(P_k,\hat{P})=\lim_{k\to\infty}\int_0^LD(P_p,\hat{P})dX=\mathcal{D}(P,\hat{P}),
\]
where last inequality holds because $D$ is finite and continuous on $[p\,infty)\times[p,\infty)$ by (d3). This concludes the proof.
\end{proof}
...
We consider the following boundary value problem: the bar in not subject to any distributed load, that is $f=0$. Moreover the end sections are left free to slide in the transversal direction, this means that $h_2(X)=0$ for $X\in\{0,L\}$ and $\Gamma_D^2=\emptyset$. The longitudinal position of the end sections is assigned, this means that $\Gamma_D^1=\{0,L\}$. In particular we consider the boundary conditions $\chi_1(0)=0$ and $\chi_1(L)=a(t)L$, where $a\colon[0,T]\to[1,\infty)$ is the imposed elongation.
...
\begin{subequations}
\begin{gather}
W_{11}^*u_1''+W_{12}^*u_2'+W_{1G}^*u_2''=0\quad\text{in $(0,L)$} \\
\begin{multlined}[c]
-W_{12}^*u_1'-W_{22}^*u_2-W_{2G}^*u_2'+\\
+W_{1G}^*u_1''+W_{2G}^*u_2'+W_{GG}^*u_2''=0
\end{multlined}
\quad\text{in $(0,L)$} \\
W_{1G}^*u_1'(X)+W_{2G}^*u_2(X)+W_{GG}^*u_2'(X)=0\quad\text{for $X\in\{0,L\}$}.
\end{gather}
\end{subequations}
...
\begin{subequations}
\begin{gather}
W_{11}^*u_1''+W_{12}^*u_2'=0\quad\text{in $(0,L)$} \\
-W_{12}^*u_1'-W_{22}^*u_2+W_{GG}^*u_2''=0 \quad\text{in $(0,L)$} \\
...
\begin{subequations}
\begin{gather}
W_{GG}^*u_2'''+\left(\frac{(W_{12}^*)^2}{W_{11}^*}-W_{22}^*\right)u_2'=0 \quad\text{in $(0,L)$} \\
u_2'(X)=0\quad\text{for $X\in\{0,L\}$}.
\end{gather}
\end{subequations}
Let $\xi=u_2'$ and
