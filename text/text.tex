...
\[
\psi=\frac{1}{2}\kappa(e_1+(d-1)e_2)^2+\frac{1}{2}\mu(e_1-e_2)^2+\frac{1}{2}l^2\mu|u_2'|^2+\frac{1}{2}Hp^2.
\]
and the dissipation potential for unit axial length
\[
\phi=R(\dot p)
\]
...
\tau\in Hp+\partial R(\dot p).
\end{gather*}
\begin{lemma}
Every process that obeys the constitutive relations satisfy the dissipation inequality.
\end{lemma}
\begin{proof}
Let $\tau=Hp+\xi$ with $\xi\in\partial R(\dot p)$. The dissipation per unit axial length is given by
\[
...
\subsection{Traction with stick extremities in the elastic range}
