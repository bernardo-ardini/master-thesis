...
We start describing the kinematics of the bar from a multidimensional perspective. This will serve as a motivation for the development of the purely one dimensional model discussed in the forthcoming sections.
...
The space $Z$ of the configurations $z=(u,p)$ will be clearly defined subsequently. The virtual velocities will be denoted as $\tilde w=(\tilde v,\tilde q)$

\subsection{Statics}

To describe the coupling between the macroscopic configuration $u$ and the microscopic configuration $p$ we require that the internal generalized force is the form
\[
\dual{\mathcal I,\tilde w}=\int_0^L\zeta\tilde v_2'dx+\int_0^L\left(\sigma_1(\tilde v_1'-\tilde q)+\sigma_2(\tilde v_2+(d-1)^{-1}\tilde q)\right)dx+\int_0^L\tau\tilde qdx.
\]
The first integral describes a purely macroscopic response.. Here $\zeta\colon(0,L)\to\mathbb{R}$ is the transversal stress. The second integral describes the elastic response that couples the macroscopic and the microscopic configurations. Here $\sigma_1\colon(0,L)\to\mathbb{R}$ is the longitudinal elastic stress and $\sigma_2\colon(0,L)\to\mathbb{R}$ the longitudinal elastic force. The last integral describes the microscopic response and $\tau\colon(0,L)\to\mathbb{R}$ is the microscopic force.
...
\phi=R(\dot p)
\end{equation}
where $R\colon\mathbb{R}\to[0,\infty]$ is convex and satisfy $R(0)=0$. A possible choice could be $R(\dot p)=Y|\dot p|$ where $Y>0$ is the yield strength. Consistently with these choices and~\eqref{eqn:modeling:constitutive}, we impose the following constitutive relations
\begin{gather*}
\zeta=\mu l^2u_2' \\
...
As we showed in the previous chapter, the one dimensional small deformations model of a bar is not able to predict necking. This leads us to refine the one dimensional model allowing large deformations.
...
\end{proof}
We will see particular examples of constitutive relations in the next section, after we have clarified which assumptions are required for the model to be well-posed.

\section{Existence result}
\label{sec:existence-necking}

In this section we prove the existence of energetic solutions. A self-contained introduction to this solution concept, including an explanation of the notation employed in this section, can be found in Section~\ref{sec:energetic-solution} of Appendix~\ref{chap:solution-concepts}. The aim of this section is to verify the hypotheses required to apply Theorem~\ref{thm:existence-energetic-solutions}.
...
\]
where $A,B,l>0$, $r>1$, $0<\alpha\le p-1$ and $p>1+\frac{\alpha}{r}$. We now check that this particular $W$ satisfies (W1), (W2), (W3) and (W4) of previous section. We start with the check of the assumptions (W1) and (W2). Hypotheses (W1) is a consequence of the following lemma.
\begin{lemma}
...
\begin{rmk}
Most of the stability analysis below is only formal. Some additional effort should be made in order to get fully rigorous results. The mathematical theory that should be employed to reach a full justification of the procedure below is that of bifurcation theory in Banach spaces. Some results regarding this theory and application to instability phenomena in nonlinear elasticity can be found in our review~\cite{progetto-ponno}.
\end{rmk}
...
\begin{figure}
\begin{tikzpicture}
\begin{axis}[xlabel={$a$},ylabel={$\overline F$},xmin=1,xmax=6,ymin=0,width=0.45\textwidth,ylabel style={rotate=-90}]
\addplot[-] table [x=E1,y=F] {figures/instability.dat};
\end{axis}
\end{tikzpicture}
\hfill
\begin{tikzpicture}
\begin{axis}[xlabel={$a$},ylabel={$\lambda$},xmin=1,xmax=6,width=0.45\textwidth,ylabel style={rotate=-90}]
...
the thesis follows.
\end{proof}
It is easy to see that the constitutive model described in Section~\ref{subsec:constitutive-large-neck} implies that $\det b(t)>0$. Then the sign of $dF/d\eta$ coincides with the sign of $\det h$. This suggest that $h$ is positive definite when $F$ increases and became indefinite if $F$ reaches a maximum. This is indeed confirmed by the numerical computation of the trivial solution.

We notice that the constitutive equation of the hardening free energy proposed in Section~\ref{subsec:constitutive-large-neck} gives a monotone relation between $F$ and $\eta$ if $C$ is large enough, see the dashed graph in Figure~\ref{fig:instabilitypl}. According to the considerations above we expect that hardening does not occur in this case. When $C>0$ is sufficiency reduces a non monotone relation is obtained, see the solid graph in Figure~\ref{fig:instabilitypl}.
...
\begin{figure}[htp]
\begin{tikzpicture}
\begin{axis}[xlabel={$a$},ylabel={$F^*$},xmin=1,xmax=1.5,width=0.45\textwidth,ylabel style={rotate=-90}]
\addplot[-] table [x=dchi1,y=F] {figures/instabilitypl.dat};
\addplot[dashed] table [x=dchi1,y=F] {figures/instabilitypl-i.dat};
\end{axis}
\end{tikzpicture}
