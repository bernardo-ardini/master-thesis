...
With the same reasoning that lead to choose an internal power of the form~\eqref{eqn:powers-plasticity} in the multidimensional plasticity theory discussed in Section~\ref{sec:plasticity-review}, we assume that the generalized internal force in out one-dimensional model is of the form
\[
...
We now proceed to develop a purely one-dimensional model of the bar. This model is formulated independently of the fully multidimensional one; therefore, the discussion in the previous section should be viewed merely as a motivation for the specific choices made in the following derivation.
...
We will see that a trivial strong solution to this problem is the spatially homogeneous one that is $\bar{y}_1(X,t)=X$ for all $X\in(0,L)$ and $t\in[0,T]$, $\bar{y}_1(\cdot,t)$ and $\bar{P}(\cdot,t)$ constant for all $t\in[0,T]$. However we expect that this trivial solution is unstable when $\eta$ is sufficiently large, in the sense that it does not satisfy the global stability condition
\[
...
\]
Then the following trivial fact holds.
\begin{lemma}
...
\end{gather*}
then it is a local minimum of $\mathcal{J}_t$ in the following sense
\[
\dual{\partial^+\mathcal{J}_t(\overline{q}),w}>0\quad\text{or}\quad\begin{cases}
\dual{\partial^+\mathcal{J}_t(\overline{q}),w}=0 \\
\dual{\partial^+\partial^+\mathcal{J}_t(\overline{q})w,w}>0
\end{cases}\qquad\text{for all $w\in Q$}.
\]
\end{lemma}
\begin{proof}
That~\eqref{eqn:min-neck} implies~\eqref{eqn:min-neck-sigma} is trivial.
\end{proof}
%\begin{lemma}
...
%\end{proof}
Thus a sufficient condition for instability is that one among~\eqref{eqn:min-neck-sigma} is violated, so we now try to study when this happen.
...
\begin{rmk}
The result that we have obtained actually allows to determine a lower bound 
\end{rmk}
