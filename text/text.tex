...
F-Bp=Y.
\]
This implies $p=B^{-1}(F-Y)$.

\subsection{Traction with clamped end sections}

Suppose that $f(x,t)=0$, $h_1(L,t)=h_1(0,t)=F(t)$, and $u_2(L,t)=u_2(0,t)=0$ for all $x\in(0,L)$ and $t\in[0,T]$. By the same computations of previous section we find the equation
\[
l^2\mu u_2''=\frac{(d-1)\kappa-\mu}{\kappa+\mu}F+\frac{d^2}{d-1}\frac{\kappa\mu}{\kappa+\mu}p+d^2\frac{\kappa\mu}{\kappa+\mu}u_2.
\]
...
\]
Then taking into account the boundary condition we find the solution
\[
u_2=\frac{(d-1)\kappa-\mu}{d^2\kappa\mu}\left(\frac{\cosh\left(d\sqrt{\frac{\kappa}{\kappa+\mu}}\frac{x-\frac{L}{2}}{l}\right)}{\cosh\left(d\sqrt{\frac{\kappa}{\kappa+\mu}}\frac{L}{2l}\right)}-1\right)F.
\]
To simplify the notation we define
\[
Z(x)=\frac{\cosh\left(d\sqrt{\frac{\kappa}{\kappa+\mu}}\frac{x-\frac{L}{2}}{l}\right)}{\cosh\left(d\sqrt{\frac{\kappa}{\kappa+\mu}}\frac{L}{2l}\right)}\quad\text{and}\quad E=\frac{d^2\kappa\mu}{(d-1)\kappa-\mu}.
\]
Then
\[
u_2=e_2=(Z-1)\frac{F}{E}.
\]
We find that the Mandel stress is
\[
m=
\]

Assume that the cylindrical bar has a purely elastic response. Then the only unknowns are $u_1$ and $u_2$. Suppose that $f(x,t)=0$, $h_1(L,t)=h_1(0,t)=F(t)$, and $u_2(L,t)=u_2(0,t)=0$ for all $x\in(0,L)$ and $t\in[0,T]$. From equation~\eqref{eqn:local-form-1} and~\eqref{eqn:local-form-2} we have $\sigma_1=F$. Then
\[
\kappa(u_1'+(d-1)u_2)=F-\mu(u_1'-u_2)\quad\text{and}\quad u_1'=\frac{F-(d-1)\kappa u_2+\mu u_2}{\kappa+\mu}.
\]
From~\eqref{eqn:local-form-3} we get
\[
-(d-1)\left(F-\mu(u_1'-u_2)\right)+\mu(u_1'-u_2)+l^2\mu u_2''=0
\]
\[
-(d-1)\left(F-\frac{\mu}{\kappa+\mu}(F-d\kappa u_2)\right)+\frac{\mu}{\kappa+\mu}(F-d\kappa u_2)+l^2\mu u_2''=0
\]
\[
-(d-1)\frac{\kappa}{\kappa+\mu}\left(F+d\mu u_2\right)+\frac{\mu}{\kappa+\mu}(F-d\kappa u_2)+l^2\mu u_2''=0
\]
\[
\frac{\mu-(d-1)\kappa}{\kappa+\mu}F-d^2\frac{\kappa\mu}{\kappa+\mu}u_2+l^2\mu u_2''=0
\]
\[
u_2''=\frac{d^2}{l^2}\frac{\kappa}{\kappa+\mu}u_2+\frac{(d-1)\kappa-\mu}{l^2\mu(\kappa+\mu)}F.
\]
A solution of the homogeneous equation which is also symmetric with respect to $x=L/2$ is
\[
x\mapsto\cosh\left(d\sqrt{\frac{\kappa}{\kappa+\mu}}\frac{x-\frac{L}{2}}{l}\right).
\]
