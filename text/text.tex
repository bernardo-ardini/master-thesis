...
We now describe how the model formulated in the previous section, can be treated within the realm of energetic solutions.
...
We now discuss the assumptions with which we will prove the validity of hypotheses (E1)-(E4), (D1)-(D2) and (C1)-(C2) of Section~\ref{sec:energetic-solution} in Appendix~\ref{chap:solution-concepts}. Subsequently, these assumptions will be implicitly supposed to be valid and will not be recalled.
...
\end{itemize}
We do not know whether the proof works without ($\hat{\text{G}}$).

The elastic free energy density is supposed to satisfy the following assumptions
\begin{itemize}[itemsep=1em]
\item[(W1)] there exists $\mathbb{W}\colon\mathbb{R}^4\to(-\infty,\infty]$ continuous and convex such that
\[
W(E_1,E_2,G)=\mathbb{W}(E_1,E_2,G,E_1E_2)
\]
for all $E_1,E_2,G\in\mathbb{R}$,
...
\begin{itemize}
\item[(\"O)] it holds $\frac{1}{q_Y}=\frac{1}{q_W}+\frac{1}{q_H}$.
\end{itemize}
...
\end{gather*}
Assume also that $P_k,P\ge p>0$ a.e.\ in $(0,L)$. Let also $\gamma_1^k,\gamma_1,\gamma_2^k,\gamma_2,\rho_2^k,\rho_2\in L^\infty((0,L))$ such that
\begin{align*}
\gamma_1^k\to\gamma_1\quad\text{in $L^\infty((0,L))$} \\
\gamma_2^k\to\gamma_2\quad\text{in $L^\infty((0,L))$} \\
\rho_2^k\to\rho_2\quad\text{in $L^\infty((0,L))$}
\end{align*}
Then, up to extracting a subsequence,
\begin{align*}
\gamma_1^k(y_1^k)'P_k^{-1}&\rightharpoonup \gamma_1y_1'P^{-1}\quad&\text{in $L^1((0,L))$} \\
\gamma_2^ky_2^kP_k&\rightharpoonup \gamma_2y_2P\quad&\text{in $L^1((0,L))$} \\
...
By Rellich--Kondrachov theorem, up to extracting a subsequence, $y_1^k\to y_1$ and $P_k\to P$ in $C^0([0,L])$.  Moreover $g_1'(y_1^k,t)\to g_1'(y_1,t)$, $g_2(y_1^k,t)\to g_2(y_1,t)$ and $g_2'(y_1^k,t)\to g_2'(y_1,t)$ in $C^0([0,L])$. Indeed if $\gamma\in C^1(\mathbb{R})$ then
\begin{equation}
...
g_2(y_1^k,t)y_2^kP_k&\rightharpoonup g_2(y_1,t)y_2P\quad&\text{in $L^1((0,L))$} \\
...
\subsection{Verification the properties of the dissipation potential}
...
We consider the following boundary value problem: the bar in not subject to any distributed load, that is $f=0$. Moreover the end sections are left free to slide in the transversal direction, this means that $h_2(X)=0$ for $X\in\{0,L\}$ and $\Gamma_D^2=\emptyset$. On the contrary the longitudinal position of the end sections is assigned, this means that $\Gamma_D^1=\{0,L\}$. In particular we consider the boundary conditions $\chi_1(0)=0$ and $\chi_1(L)=E_1^*(t)L$, where $E_1^*\colon[0,T]\to[1,\infty)$ is the imposed elongation.

\subsection{Purely elastic case}

In the purely elastic case, equations~\eqref{eqn:indeinite-equation-necking-large} reduces to
\begin{subequations}
...
\end{gather}
\end{subequations}
We first look for homogeneous solutions of the form $\chi_1^*(X)=E_1^*X$, $\chi_2^*(X)=E_2^*$ where $E_1^*=E_1^*(t)$ is assigned and $E_2^*\in(0,\infty)$ is unknown. Notice that $G^*=(\chi_2^*)'=0$. Then $E_2^*=E_2^*(E_1^*)$ must satisfy the nonlinear equation
\[
W_2(E_1^*,E_2^*,0)=0.
\]
...
Let $\xi=u_2'$ and
\[
\lambda=\lambda(E_1^*)=\frac{\frac{(W_{12}^*)^2}{W_{11}^*}-W_{22}^*}{W_{GG}^*}.
\]
...
\begin{rmk}
Let $F^*=F^*(E_1^*)=W_1^*$ be the axial force obtained in the homogeneous solution. Then
\[
\frac{dF^*}{dE_1^*}=W_{11}^*+W_{12}^*\frac{dE_2^*}{dE_1^*}.
\]
On the other end we know that $W_2^*=0$ so that, by differentiating with respect to $E_1^*$,
\[
W_{12}^*+W_{22}^*\frac{dE_2^*}{dE_1^*}=0\quad\text{that implies}\quad\frac{dE_2^*}{dE_1^*}=-\frac{W_{12}^*}{W_{22}^*}.
\]
Then
\[
\frac{dF^*}{dE_1^*}=W_{11}^*-\frac{(W_{12}^*)^2}{W_{22}^*}=-W_{GG}^*\frac{W_{11}^*}{W_{22}^*}\lambda(E_1^*).
\]
...
\begin{lemma}
Let $r\ge1$. Then there exists $C>0$ such that $|x-1|^r\ge C|x|^p-1$ for all $x\in\mathbb{R}$.
\end{lemma}
\begin{proof}
By the equivalence of the norms there exists $K>0$ such that
\[
(|x-1|^p+1)^{1/p}\ge K(|x-1|+1)\ge K(|x|-1).
\]
Now the thesis follows immediately.
\end{proof}

Thus we have that $W$ satisfies the required assumptions. Now we show that $W$ provides homogeneous solutions that are likely to become unstable. Indeed choosing particular values of the parameter appearing in the definition of $W$, we get the graphs of $F^*$ and $\lambda$ as functions of $E_1^*$ in Figure~\ref{fig:instability}. In particular notice that $\lambda$ became positive as $E_1^*$ is increased. This implies that, if $L$ is sufficiently large, then there exists $E_1^*$ such that $\lambda=\pi^2/L^2$.

\begin{figure}
\begin{tikzpicture}
\begin{axis}[xlabel={$E_1^*$},ylabel={$F^*$},xmin=1,xmax=6,ymin=0,width=0.45\textwidth,ylabel style={rotate=-90}]
\addplot[-] table [x=E1,y=F] {figures/instability.dat};
\end{axis}
\end{tikzpicture}
\hfill
\begin{tikzpicture}
\begin{axis}[xlabel={$E_1^*$},ylabel={$\lambda(E_1^*)$},xmin=1,xmax=6,width=0.45\textwidth,ylabel style={rotate=-90}]
\addplot[-] table [x=E1,y=lambda] {figures/instability.dat};
\addplot[dashed,domain=1:6] {0};
\end{axis}
\end{tikzpicture}
\caption{Graphs of $F^*$ and $\lambda$ as functions of $E_1^*$ for the homogeneous deformation when $A=1$, $B=10^{-4}$, $p=2$, $\alpha=1$ and $r=2$. Notice that $\lambda$ became positive for $E_1$ sufficiently large, indicating that the uniform solution is likely to became unstable if $L$ is sufficiently large.}
