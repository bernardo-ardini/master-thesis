...
\]
Since $\hat{q}\in\mathscr{D}$, then (H3) implies that $\hat{P}>p$ a.e.\ in $(0,L)$, so that $\hat{P}\ge p$ in $[0,L]$. Since also $q_k\in\mathscr{D}$ then, for the same reason, also $P_k\ge p$ in $[0,L]$. Notice also that Then $P_k\to P$ in $C^0([0,T])$. Then (d3) yields that actually
\[
\lim_{k\to\infty}\mathcal{D}(P_k,\hat{P})=\mathcal{D}(P,\hat{P}).
\]
This concludes the proof.
\end{proof}
...
\begin{align*}
C\left(\frac{1}{\chi_2}-\frac{l^2(\chi_2')^2}{(\chi_1')^2\chi_2}-\frac{\chi_2}{(\chi_1')^2}\right)+\chi_2w'(\chi_1'\chi_2)&=F \\
-C\left(-\frac{\chi_1'}{\chi_2^2}-\frac{l^2(\chi_2')^2}{\chi_1'\chi_2^2}+\frac{1}{\chi_1'}\right)-\chi_1'w'(\chi_1'\chi_2)+2Cl^2\left(\frac{\chi_2'}{\chi_1'\chi_2}\right)'&=0.
\end{align*}
From the first equation we get the following third order algebraic equation for $\chi_1'$
\[
2A\chi_2^2(\chi_1')^3+\left(\frac{C}{\chi_2}-2A\chi_2-F\right)(\chi_1')^2-C\left(l^2\frac{(\chi_2')^2}{\chi_2}+\chi_2\right)=0.
\]
A classical choice for the dissipation potential is $R(L)=Y|L|$ where $Y>0$ is the yield strength.
