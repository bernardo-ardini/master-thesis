...
To describe the experimentally observed increase of the yield strength of metals at small scales, since the work of Aifantis~\cite{Aifantis1984} in 1984, has been developed plasticity models (see~\cite{FleckHutchinson1993} for the early developments) that include a dependence on the gradient of the kinematical field describing the plastic distortion. This allows to include in the model an internal characteristic length scale that is not considered in the classical plasticity. One refer to this kind of models as the gradient plasticity theory. Modern formulations are given in~\cite{gurtin-anand-small},~\cite{gurtin-anand-large} and \cite{BertramForest2020}. The importance of the gradient plasticity theory in this thesis is mainly due to a mathematical issue rather then a modeling concern. Indeed we will see that the inclusion of the dependence the plastic distortion gradient will be necessary to ensure compactness properties needed to handle successfully the nonconvex character of plasticity at large strain in Part~\ref{part:his-model} and the complex nonlinear couplings of the model discussed in Part~\ref{part:my-model}.
...
\begin{rmk}
In accordance with the discussion made in Section~\ref{sec:ppv}, we do not include the constrain $\det F>0$ (the impenetrability condition) and $\det F_p=1$ (the plastic incompressibility) in the definition of $Z$.
\end{rmk}
...
$\text{DevPla}$ & defined as $\text{DevSym}\otimes(\text{DevSym}\otimes\mathbb{R})$ \\
