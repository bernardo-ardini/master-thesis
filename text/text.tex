...
Let $Y=W^{1,q_Y}((0,L),\mathbb{R}^2)$ with $q_Y\in(??,\infty)$ and $Z=L^{q_H}((0,L))\cap W^{1,r}((0,L))$. Set $Q=Y\times Z$. PER ORA IL VINCOLO SUGLI ESPONENTI È CHE DEVONO ESSERE TUTTI MAGGIORI DI  E MINORI DI INFINITO E $\frac{1}{q_Y}+\frac{1}{q_H}<1$.

\begin{equation}
\tag{G}
\parbox{0.8\textwidth}{$g_1\in C^2(\mathbb{R}\times[0,T])$, $g_2\in C^2(\mathbb{R}\times[0,T],(0,\infty))$ and there exists $0<\gamma<\Gamma<\infty$ for $i\in\{1,2\}$ such that $\gamma<g_1'<\Gamma$, $\gamma<g_2<\Gamma$ and $|g_2'|\le\Gamma$.}
\end{equation}

\begin{gather}
\parbox{0.8\textwidth}{there exists $\mathbb{W}\colon\mathbb{R}^4\to(-\infty,\infty]$ continuous and convex such that $W(E_1,E_2,G)=\mathbb{W}(E_1,E_2,G,E_1E_2)$ for all $E_1,E_2,G\in\mathbb{R}$}
\tag{W0}
\\
\parbox{0.8\textwidth}{there exist $C_W>0$ and $q_W\in(1,\infty)$ such that $W(E_1,E_2,G)\ge C_W(-1+|E_1|^{q_F}+|E_2|^{q_F}+|G|^{q_F})$ for all $E_1,E_2,G\in\mathbb{R}$}
\tag{W1}
\\
\parbox{0.8\textwidth}{$W(E_1,E_2,G)=\infty$ if and only if $E_1\le0$ or $E_2\le0$}
\tag{W2}
\end{gather}

\begin{gather}
\parbox{0.8\textwidth}{$H\colon\mathbb{R}^2\to(-\infty,\infty]$ is continuous and $H(P,\cdot)$ is convex for all $P\in\mathbb{R}$}
\tag{H0}
\\
\parbox{0.8\textwidth}{there exist $C_H>0$ and $q_H,r\in(1,\infty)$ such that $H(P,P')\ge C_H(-1+|P|^{q_H}+|P'|^r)$ for all $P,P'\in\mathbb{R}$}
\tag{H1}
\\
\parbox{0.8\textwidth}{there exists $p>0$ such that $H(P,P')=\infty$ if and only if $P\le p$}
\tag{H2}
\end{gather}

\begin{gather}
\parbox{0.8\textwidth}{$D\colon\mathbb{R}^2\to[0,\infty]$ is continuous}
\tag{D0}
\\
\parbox{0.8\textwidth}{there exist $C_D>0$ and $q_D\in[1,q_H)$ such that $D(P_0,P_1)\le C_D(1+|P_0|^{q_D}+|P_1|^{q_D})$}
\tag{D1}
\\
\parbox{0.8\textwidth}{$D$ is a quasidistance in the sense that $D(P_0,P_1)=0$ if and only if $P_0=P_1$ and $D(P_0,P_2)\le D(P_0,P_1)+D(P_1,P_2)$ for all $P_0,P_1,P_2\in\mathbb{R}$.}
\tag{D2}
\end{gather}
...
\begin{prop}
There exists $\mathscr{D}\subset\mathscr{Q}$ such that $\dom\mathcal{E}(\cdot,t)=\mathscr{D}$ for all $t\in[0,T]$. Moreover
\begin{itemize}
...
\parbox[c]{0.8\textwidth}{for all $t\in[0,T]$ and $E\in\mathbb{R}$ the set $\{q\in\mathscr{Q}\;|\;\mathcal{E}(q,t)\le E\}$ is bounded and weakly closed}
\tag{E1}
\\
...
\parbox[c]{0.8\textwidth}{for all $q\in\mathscr{D}$ the function $\mathcal{E}(q,\cdot)$ is $C^1([0,T])$, uniformly continuous on $\mathscr{D}\times[0,T]$, and there exist $C_1^E$ and $C_0^E$ such that $|\partial_t\mathcal{E}(q,t)|\le C_1^E(C_0^E+\mathcal{E}(q,t))$ for all $q\in\mathscr{Q}$ and $t\in[0,T]$.}
\tag{E3}
\end{gather}
