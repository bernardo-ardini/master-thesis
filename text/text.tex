...
\[
\mathcal{J}_t^\varsigma(\hat y,\hat P,t)=\int_0^LW(\eta(t)\hat y_1'\hat P^{-1},\hat y_2\hat P,\hat y_2'\hat P^{-1})dX+\int_0^LH(\hat P,\hat P')dX+\int_0^LD^\varsigma(\bar P,\hat P)dt,
\]
with $D^\varsigma(P_0,P_1)=\varsigma Y\log(P_1/P_0)$. And we notice that $\mathcal{J}_t(\hat q)=\mathcal{J}_t^\pm(\hat q)$ if $\hat q\in\mathscr{Q}^\varsigma_t$ where
\[
\mathscr{Q}^\varsigma_t=\{\hat q=(\hat y,\hat P)\in\mathscr{Q}\;|\;\text{$\varsigma(\hat P-\bar P(t))\ge0$ in $(0,L)$}\}.
\]
Then the following lemma express some relations between the minimality of $\mathcal{J}_t$ and similar conditions for $\mathcal{J}_t^\varsigma$.
\begin{lemma}
...
%\end{proof}
Thus a sufficient condition for instability is that at least one among~\eqref{eqn:min-neck-sigma} is violated, so we now try to study whether this happen.

Considering the way in which the trivial solution is computed (see the beginning of Section`\ref{subsec:plastic-model}) it is immediate to check that $\partial\mathcal{J}^+_t(\bar q(t))=0$ for all $t\in[0,T]$. This implies that for all $w=(v,Q)\in Q$ with $w_1(0)=w_1(L)=0$ and $Q\le0$ in $(0,L)$, it holds that
\[
...
\]
This means that~\eqref{eqn:min-neck-sigma} for $\varsigma=-1$ is satisfied, at least locally for $\hat q$ near $\bar q(t)$, for all $t\in[0,T]$. So we turn to study whether~\eqref{eqn:min-neck-sigma}, for $\varsigma=1$, is violated. To this aim we could study the second variation of $\mathcal{J}_t^+$ at $(\bar{y}(t),\bar P(t))$: if we are able to find $w=(v,Q)\in Q$ with $w_1(0)=w_1(L)=0$ and $Q\ge0$ in $(0,L)$ such that $\dual{\partial^2\mathcal{J}_t^+(\bar q(t))w,w}<0$, we have done. Clearly the critical condition would be the existence of a nontrivial $w=(y,Q)$ with $Q\ge0$ such that $\partial^2\mathcal{J}_t^+(\bar q(t))w=0$. This last equation is the linerization of the perturbed equation $\partial\mathcal{J}_t^+(\bar q(t)+w)=0$. Since our efforts to find a nontrivial solution $w$ satisfying the constraint $Q\ge0$ were unsuccessful, in what follows we presents a simplified analysis in which that constraint is disregarded. This leads us to the following second simplified notion of stability.
...
\end{defi}
So, now we study the perturbed equation equation $\partial^2\mathcal{J}_t^+(\bar q(t))w=0$. After some tedious computation we wind that (for each $t$ fixed that we will not specify anymore) this is equivalent to
\begin{equation}
\label{eqn:2order}
Aw''+Bw'+Cw=0\quad\text{in $(0,L)$},
\end{equation}
together with the boundary conditions $w_1(X)=0$, $w_2'(X)=0$ and $w_3'(X)=0$ for $X\in\{0,L\}$. We have used the notation $v=(w_1,w_2)$ and $Q=w_3$ and we have defined
\begingroup
\tiny
\begin{subequations}
...
-b_2 & 0 & 0
\end{pmatrix}= \begin{pmatrix}
    0 & \bar{W}_{y'_1 y_2} & \bar{W}_{y'_1 \bar{P}} \\
    -\bar{W}_{y'_1 y_2} & 0 & \bar{W}_{y'_2 \bar{P}} \\
    -\bar{W}_{\bar{P} y'_1} & -\bar{W}_{\bar{P} y'_2} & 0
\end{pmatrix}=\begin{pmatrix}
...
\end{pmatrix}=\begin{pmatrix}
    ...
    0 & -\bar{W}_{\bar{P} y_2} & -\bar{W}_{\bar{P} \bar{P}} - \bar{H}_{11} - \bar{D}_{22}
\end{pmatrix}=
\begin{pmatrix}
0 & 0 & 0 \\
0 & -\bar{P}^2 \bar{W}_{22} & -\bar{W}_2 + \frac{\eta}{\bar{P}} \bar{W}_{12} - \bar{y}_2 \bar{P} \bar{W}_{22} \\
0 & -\bar{W}_2 + \frac{\eta}{\bar{P}} \bar{W}_{12} - \bar{y}_2 \bar{P} \bar{W}_{22} & -\eta^2 \frac{\bar{W}_{11}}{\bar{P}^4} - \bar{y}_2^2 \bar{W}_{22} - 2\eta \frac{\bar{W}_1}{\bar{P}^3} + 2\eta \frac{\bar{y}_2}{\bar{P}^2} \bar{W}_{12} - \bar{H}_{11} - \bar{D}_{22}
\end{pmatrix}
\end{align}
\end{subequations}
\endgroup
...
\end{gather}
\end{subequations}
in $(0,L)$. Taking into account the boundary conditions we can write $w_1$, $w_2$ and $w_3$ as the following formal series
\[
...
\end{gather}
\end{subequations}
in $(0,L)$, where. Taking into account the orthogonality relations
\[
...
\end{cases}
\]
then~\eqref{eqn:series} is equivalent to $M_kw^k=0$ for all $k\in\mathbb{N}_0$ where $w^0=(w_2^0;w_3^0)\in\mathbb{R}^2$, $w^k=(w_1^k;w_2^k;w_3^k)\in\mathbb{R}^3$ for $k\in\mathbb{N}$ and
\[
M_0=\begin{pmatrix}
c_1 & c_2 \\
c_2 & c_3
...
\label{fig:dets}
\end{figure}
When $\eta=\eta_1$ the kernel of $\partial^2\mathcal{J}^+_t(\overline q(t))$ becomes nontrivial and is spanned by $w$
\[
w_1(x)=w_1^1\sin(\gamma_1x),\quad w_2(x)=w_2^1\cos(\gamma_1x)\quad\text{and}\quad w_3(x)=w_3^1\cos(\gamma_1x)
\]
with $w^1$ satisfying $M_1w^1=0$. If we compote numerically $w^1$ we find that $w^1>0$, $w^2<0$ and $w^3>0$, so interpreted as a variation of the configuration of the bar it appears qualitatively as the instability mode we found in the purely elastic case and reported in Figure~\ref{fig:instability-mode-elastic}. Even though $w$ belongs to the kernel of $\partial^2\mathcal{J}_t^+$, it is not necessarily a true instability mode because, since it does not satisfies $w_3\ge 0$ does not necessarily made stationary the second variation of the true objective functional $\mathcal{J}_t$.
