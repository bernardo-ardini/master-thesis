...
Some historical information about the birth of the principle of virtual powers can be found in~\cite{ppv-encyclopedia} and in~\cite{ppv-maugin}. Modern works that use the principle of virtual powers to develop systematically mechanical model are~\cite{germain},~\cite{gurtin-fried}, \cite{gurtin-fried-turbulence}, where second gradient theories are considered, and~\cite{gurtin-anand-small}, \cite{gurtin-anand-large}, \cite{non-smooth-thermomechanics} that treat materials with internal variables.
...
\end{prb}
\begin{rmk}
Before imposing particular constitutive relations, $\mathcal{I}$ and $\mathcal{X}$ are completely undefined. However we notice that, before making specific constitutive assumptions, it is useful to assume that $\mathcal{I}$ and $\mathcal{X}$ have a more specific structure as we will seen in the following chapters. We notice that, in order to keep the notation simple, we will sometimes write $\tilde{\mathcal{S}}$ instead of $\dual{\mathcal{S},\tilde v}$ for $\mathcal{S}\in\{\mathcal{I},\mathcal{X},\mathcal{F}\}$.
...
\end{rmk}
\begin{rmk}
In terms of $\Phi$, the dissipative inequality is
\begin{equation}
