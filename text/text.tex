...
We have thus proved that when $\eta\ge\eta_1$ the trivial solution does not satisfy the regularized stability condition. When $\det M_1$ vanishes, the kernel of $\partial^2\mathcal{J}^+_t(\overline q(t))$ becomes nontrivial and is spanned by $w$
\[
...
We have proved that the kernel of $\partial\mathcal{J}^+_t(\bar{q}(t))$ never contains $w\in Q$ with $Q\ge 0$. However to prove that $\bar{q}(t)$ is not a minimizer of $\mathcal{J}^+_t(\overline{q}(t))$ for some $\eta>\eta_1$ it would be sufficient to show that such $w$ exists with the property that $\dual{\mathcal{J}^+_t(\bar{q}(t))w,w}<0$. Here we describe our attempts in that direction. Consider a finite dimensional subspace of $Q$ spanned by $\{w^1,\dots,w^m\}$ that we collect in the matrix field $O\colon(0,L)\to\mathbb{R}^{3\times m}$ given by $O=(w_1,\dots,w_m)$. Then generic element of that subspace is $w=Oc$ where $c\in\mathbb{R}^m$. Thus
\[
\dual{\mathcal{J}^+_t(\overline{q}(t))w,w}=c^TG(t)c,
\]
where the symmetric matrix $G\in\mathbb{R}^{d\times d}$ is given by
\[
G=\int_0^L\left((O')^TMO'+2(O')^TNO+O^TKO\right)dX.
\]
Thus out aim is to find $O$ and $\eta>\eta_1$ such that there exists $c\in\mathbb{R}^{m}$ with $c^TGc<0$ and satisfying the constrain $(Oc)_3\ge0$ in $(0,L)$. We first consider the case
\[
O(x)=\begin{pmatrix}
\sin(\gamma_1x) & 0 & 0 \\
0 & 1 & 0 \\
...
0 & 0 & \cos(\gamma_1x) 
\end{pmatrix}.
\]The condition $(Oc)_3\ge0$ in this case reads as $|c_5|<c_4$. We find that 
\end{rmk}
