...
\]
The aim of what follows is to study the conditions under under which there exists $t\in(0,T]$ such that~\eqref{eqn:stability-necking} is violated and understand qualitatively the behavior of the true stable solution. We will show that choosing suitably the constitutive relations, there exists $t_0\in(0,T]$ such that, if $t>t_0$ then~\eqref{eqn:stability-necking} is violated. To see this we will look at the behavior of $\mathcal{J}_t$ locally near $(\overline y(t),\overline P(t))$ studying its first and second variations on the directions in which it is smooth (notice indeed that $\mathcal{J}_t$ is non-smooth due to the non-smooth dissipation distance $\mathcal{D}$). It will result that the direction in which $\mathcal{J}_t$ decreases, preventing $(\overline y(t),\overline P(t))$ to be a minimizer, corresponds to an instability mode qualitatively similar to that we found in the purely elastic model and shown in Figure~\ref{fig:instability-mode-elastic}. This is interpreted as the onset of a neck in one of the extremities of the bar.
\begin{rmk}
Out study of local stability does not exclude that~\eqref{eqn:stability-necking} is violated also for $t_0\le t$, so that $t_0$ must be intended as an upper bound for the instant in which instability occur. Indeed it could exist a $t\le t_0$ such that $(\overline y(t),\overline P(t))$ is a local minimizer of $\mathcal{J}_t$ but not a global minimizer. In this case the configuration bar would jump from the trivial configuration to another configuration energetically favored. However the experimental observation and the theoretical and numerical studies indicate in the introduction of Chapter~\ref{chap:intro-neck} suggest that necking happen without jumps. Thus we expect that our threshold $t_0$ of instability detected by the local analysis actually corresponds with the true instant at which necking starts. Even thought the energetic solution exhibits a jumps, this should be considered as a defect of the energetic solution concept as discussed in Chapter~\ref{chap:solution-concepts}.
\end{rmk}



To study the stability of the trivial solution we should to check if
\begin{equation}
\label{eqn:stability-necking}
\mathcal{E}(\overline{y}(t),\overline{P}(t),t)\le\mathcal{E}(\hat y,\hat P,t)+\mathcal{D}(\overline{P}(t),\hat P)\quad\text{for all $(\hat y,\hat P)\in\mathscr{Q}$}.
\end{equation}
This since $\mathcal{D}$ is a quasidistance, and in particular $\mathcal{D}(P_0,P_1)\ge0$ and equality holds if and only if $P_0=P_1$, the stability condition is equivalent to the requirement that $(\overline{y}(t),\overline P(t))$ is a global minimizer of $(y,P)\mapsto\mathcal{E}(y,P,t)+\mathcal{D}(\overline{P}(t),P)=:\mathcal{J}_t(y,P)$ in $\mathscr{Y}\times\mathscr{Z}$. We expect that~\eqref{eqn:stability-necking} is violated when $t$ overcome a threshold $t_0>0$, so that the trivial solution becomes unstable and the necking starts.

La discussione che segue è dedicata ad una analisi locale di~\ref{eqn:stability-necking}, nel senso che, invece di controllare l'esistenza di competitori $(y,P)\in\mathscr{Q}$ generici, studieremo se in un intorno di $(\overline y(t),\overline P(t))$ esistono competitori che violano la disuguaglianza di stabilità. Per fare questo potremo sostanzialmente studiare le variazioni prima e seconda di $\mathcal{J}_t$ in $(\overline y(t),\overline{P}(t))$. Siccome $\mathcal{J}_t$ non è differenziabile per via della presenza della distanza di dissipazione non-liscia $\mathcal{D}$, dovremo. Il risultato finale a cui giungeremo è che, scegliendo opportunamente le relazioni costitutive, esiste un $t_0>0$ tale che~\eqref{eqn:stability-necking} viene violata da competitors vicini a $(\overline y(t),\overline P(t))$. Chiaramente l'analisi locale che faremo fornirà una stima inferiore 

The discussion that follows is devoted to a local analysis of~\ref{eqn:stability-necking}.



We will show that, choosing suitable constitutive relations, there exists an $\eta_0>1$ such that $\mathcal{J}_t$

 the uniform solution losses. The objective of this section is to show that this expectation is

\subsubsection*{Some heuristic considerations}

We were not able to get conclusive results in this sense, so we now propose an heuristic consideration that suggest that instability in the form of necking is likely to occur if suitable constitutive relation for the hardening free energy density are chosen. 
...
The constitutive equations of the hardening free energy proposed in Section~\ref{subsec:constitutive-large-neck} gives a monotone relation between $\overline F$ and $\eta$ if $C$ is large enough, so that the above considerations suggest that necking does not occur in this case, while when $C>0$ a non monotone relation is obtained, see the the dashed lines in Figure~\ref{fig:instabilitypl}.
