...
\section{General procedure to building mechanical models}

In this section we describe a general procedure for building mechanical models that are thermodynamically coherent. This procedure will be systematically used throughout this thesis. The formulation we will give is based on the presentations in~\cite{gurtin-anand-fried},\cite{gurtin-anand-small} and~\cite{gurtin-anand-large}.
...
\]
In particular $F_{i\alpha}$ is the $i$th component of the force acting on the $\alpha$th. Since it is useful for understanding more complex models, let $N=2$, set $\tilde v_i\in\mathbb{R}^d$ with $(\tilde v_\alpha)_i=\tilde v_{\alpha i}$, and consider
\[
...
\end{ex}
Consider now a time interval $[0,T]$ and actual evolution of the system $u\colon[0,T]\to V$. For simplicity, in this discussion we assume that $u\in\text{AC}([0,T],V)$ even thought less regular evolutions, for example allowing jumps, can be considered as we will see subsequently in this thesis. During the time interval $[0,T]$ a generalized force $\mathcal{F}\colon[0,T]\to V^*$ will act on the mechanical system. We require that equilibrium holds, that is
\[
...
\end{equation}
Equation~\eqref{eqn:modeling:ppv} is the statement of the principle of virtual powers, that is, the power of the generalized force acting on the system along all virtual rates must vanish.

\subsection{Constitutive relations}

To get an evolution equation, able to predict the evolution of the mechanical system, it is necessary to link $\mathcal{F}$ to the evolution $u$. This is done by specifying a constitutive relation, that is
\[
...
\]
where $\mathcal{E}\colon V\times V\times[0,T]\to(-\infty,\infty]$ and $\mathcal{R}\colon V\times V\times[0,T]\to[0,\infty]$ are called respectively the energy and the dissipation potential. The $\partial$ symbol can have different definitions according to the regularity of $\mathcal{E}$ and $\mathcal{R}$. If Fréchet differentiability is assumed then $\partial$ denotes the Fréchet differential while if only convexity is assumed it denotes the subdifferential. Less regularity requires to use extended notions of subdifferentials such as the Fréchet subdifferential.



In most situations one can clearly split $\mathcal{F}=\mathcal{X}-\mathcal{I}$ where $\mathcal{X}$ and $\mathcal{I}$ form $[0,T]$ to $V^*$ are the external generalized force and the internal generalized force. The internal generalized force is due to the internal response of the mechanical system, while the external generalized force represents the external actions imposed on it. In this case the constitutive relations are usually specialized to
\begin{itemize}
\item[(i)] $\mathcal{E}\colon V\times[0,T]\to(-\infty,\infty]$ of the form $\mathcal{E}(u,t)=\Psi(u)-\dual{\mathcal{X}(t),u}$ where $\Psi\colon V\to(-\infty,\infty]$ is the free energy and $\mathcal{X}\colon[0,T]\to V^*$ is the external generalized force,
\item[(ii)] $\mathcal{R}(u,v,t)=\Phi(u,v)$ where $\Phi\colon V\times V\to[0,\infty]$.
\end{itemize}
...
\subsubsection*{Kinematics} Consider a general mechanical system. Its kinematical configuration is described by elements $u\in V$ of a vector space $V$.

\subsubsection*{Statics} The statics concerns with the instantaneous equilibrium between the internal and external action insisting on the configuration of the systems. The internal and external actions on the configuration of the system are described as a linear functionals, $\mathcal{W}\in V^*$ and $\mathcal{P}\in V^*$ respectively. The equilibrium between internal and external actions is written as $\mathcal{W}=\mathcal{P}$ and this can be written as
\[
\tilde{\mathcal{W}}\doteq\dual{\mathcal{W},\tilde v}=\dual{\mathcal{P},\tilde v}\doteq\tilde{\mathcal{P}}\quad\text{for all $\tilde v\in V$}
\]
which is the classical statement of the principle of virtual powers. We refer to the test vector $\tilde v\in V$ as the virtual velocity and to $\tilde{\mathcal{W}}$ and $\tilde{\mathcal{P}}$ as the virtual internal end external power respectively.

\subsubsection*{Constitutive relations}

While the external actions $\mathcal{P}$ are assigned as a function $\mathcal{P}\colon[0,T]\to V^*$, the internal actions are put in relation with the kinematics of the system through constitutive relations. Since we always neglect inertial effects we assume a constitutive relation giving the internal actions as a set valued map $\mathcal{W}\colon V\times V\rightsquigarrow V^*$ of $z$ and $\dot z$. To obtain constitutive relations compatible with thermodynamics we postulate the existence of a
\begin{itemize}
\item[(i)] free energy functional $\Psi\colon V\to(-\infty,\infty]$ which is differentiable on its domain,
\item[(i)] subdifferentiable dissipation potential $\Phi\colon V\times V\to[0,\infty]$ such that $\Phi(z,\cdot)$ is convex and $\Phi(z,0)=0$,
\end{itemize}
and we define
\[
...
\part{Existence results for a one-dimensional model of a plastic cylindrical bar}
...
\[
\tilde{\mathcal W}=\int_0^L\zeta\tilde v_2'dx+\int_0^L\left(\sigma_1(\tilde v_1'-\tilde q)+\sigma_2(\tilde v_2+(d-1)^{-1}\tilde q)\right)dx+\int_0^L\tau\tilde qdx.
\]
...
\[
\tilde{\mathcal P}=\int_0^Lf\cdot\tilde vdx+h(L)\cdot\tilde v(L)-h(0)\cdot\tilde v(0).
\]
\begin{prop}
...
\]
Let $\mathcal{R}\colon\boldsymbol{Z}\to\mathbb{R}\cup\{\infty\}$ be convex, proper, l.s.c., 1-homogeneous and Lipchitz continuous on its domain. Let $\mathcal{F}\in H^1((0,T),\boldsymbol{Z}^*)$ such that $\mathcal{F}(0)=0$. Then there exists a unique $z\in\text{AC}([0,T],\boldsymbol{Z})$ such that for almost every $t\in(0,T)$
\[
\dual{\mathcal{A}z(t),w-\dot z(t)}+\mathcal{R}(w)-\mathcal{R}(\dot z(t))\ge\dual{\mathscr{F}(t),w-\dot z(t)}\quad\text{for all $w\in\boldsymbol{Z}$}.
\]
\end{thm}
...
\[
\psi_e=C\left(\frac{I_e}{J_e}-2\right)+w(J)=C\left(\frac{E_1^2+G^2|Y|^2+E_2^2}{E_1E_2}-2\right)+w(E_1E_2)
\]
...
We now describe how the model formulated in the previous section, can be treated within the context of energetic solutions.
...
To complete the formulation of the initial boundary value problem, we need to specify the energy functional $\mathcal{E}\colon\mathscr{Q}\times[0,T]\to(-\infty,\infty]$ and the dissipation distance $\mathcal{D}\colon\mathscr{Z}\times\mathscr{Z}\to[0,\infty]$. \textbf{Mettere qualcosa, magari nella parte generale, per spiegare perché il modello descritto precedentemente, se espresso mediante le energetic solution, conduce a queste particolari $\mathcal{E}$ e $\mathcal{D}$.} The energy functional is defined by
\begin{multline*}
...
&=g((1-\lambda)(|x_0|,y_0)+\lambda(|x_1|,\lambda_1))<(1-\lambda)g(|x_0|,y_0)+\lambda g(|x_1|,y_1)=\\
...
\part{Existence results for a model describing pressure sensitive plastic materials}
...
In this chapter we describe the general procedure to develop a thermodynamically consistent mechanical model.
...
Let $Q$ be a separable and reflexive Banach space. Let $\mathcal{E}\colon Q\times[0,T]\to\mathbb{R}$ G\^ateaux differentiable such that $\mathcal{E}(\cdot,t)$ is convex for all $t\in[0,T]$ and let $\Phi\colon Q\times[0,\infty]$ be convex, lower semicontinuous and positively 1-homogeneous. Consider the following doubly nonlinear equation
\begin{equation}
...
\end{gather}
\end{subequations}
This conditions makes sense also if $\mathcal{E}$ is not G\^ateaux differentiable and $q$ is not absolutely continuous.
